...
async function getStatus(_req, env) {
  try {
    ...
    const totalProviders = await db.prepare("SELECT COUNT(*) as count FROM providers WHERE active = 1").first();
    const providersWithErrors = await db.prepare("SELECT COUNT(*) as count FROM providers WHERE active = 1 AND (needs_review = 1 OR last_error IS NOT NULL)").first();
    const providersUpToDate = await db.prepare("SELECT COUNT(*) as count FROM providers WHERE active = 1 AND last_fetch_at > datetime('now', '-1 day') AND last_error IS NULL").first();
    const totalPlans = await db.prepare("SELECT COUNT(*) as count FROM plans WHERE is_active = 1").first();
    ...
    const plansWithoutSourceUrl = await db.prepare("SELECT COUNT(*) as count FROM plans WHERE is_active = 1 AND (source_url IS NULL OR source_url = '')").first();
    const plansWithoutPrice = await db.prepare("SELECT COUNT(*) as count FROM plans WHERE is_active = 1 AND (ongoing_price_cents IS NULL OR ongoing_price_cents <= 0)").first();
    const plansWithoutSpeed = await db.prepare("SELECT COUNT(*) as count FROM plans WHERE is_active = 1 AND (speed_tier IS NULL OR speed_tier <= 0)").first();
    const uptime = null;
