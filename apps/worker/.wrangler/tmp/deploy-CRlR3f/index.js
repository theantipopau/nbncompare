...
__export(db_exports, {
  getDb: () => getDb,
  recordRunError: () => recordRunError,
  recordRunStartEnd: () => recordRunStartEnd
});
...
async function recordRunStartEnd({ started, runId, notes }) {
  const db = await getDb();
  if (started) {
    const now = (/* @__PURE__ */ new Date()).toISOString();
    const r3 = await db.prepare("INSERT INTO runs (started_at, status) VALUES (?, ?)").bind(now, "running").run();
    return r3?.meta?.last_row_id ?? null;
  } else {
    const now = (/* @__PURE__ */ new Date()).toISOString();
...
async function recordRunError(note) {
  const db = await getDb();
  const now = (/* @__PURE__ */ new Date()).toISOString();
...
var init_db = __esm({
  "src/lib/db.ts"() {
    ...
    DB = null;
    __name(getDb, "getDb");
    __name(recordRunStartEnd, "recordRunStartEnd");
    __name(recordRunError, "recordRunError");
  }
});
...
function securityHeaders() {
  return {
    "Content-Security-Policy": "default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; img-src 'self' data: https:; connect-src 'self' https://api.cloudflare.com https://api.nbndata.com.au;",
...
async function getProviders(_req) {
  const db = await getDb();
...
async function getPlans(req, env) {
  try {
    ...
    if (inMemory && inMemory.expires > now) {
      const response2 = jsonResponse({ ok: true, rows: inMemory.rows });
      response2.headers.set("X-Cache", "MEMORY");
      return response2;
    }
    if (env?.CACHE) {
      const cached = await env.CACHE.get(cacheKey);
      if (cached) {
        const response2 = jsonResponse(JSON.parse(cached));
    ...
    if (contractType) {
      q += ` AND p.contract_type = ?`;
      params.push(contractType);
    }
    if (dataAllowance) {
      q += ` AND p.data_allowance = ?`;
      params.push(dataAllowance);
    }
    if (modemIncluded === "1") {
      q += ` AND p.modem_included = 1`;
    }
    if (technologyType) {
      q += ` AND p.technology_type = ?`;
      params.push(technologyType);
    }
    if (planType) {
      q += ` AND p.plan_type = ?`;
...
var CACHE_TTL, planQueryCache, PLAN_CACHE_TTL_MS;
var init_plans = __esm({
  "src/handlers/plans.ts"() {
    ...
    init_cors();
    CACHE_TTL = 300;
    planQueryCache = /* @__PURE__ */ new Map();
    PLAN_CACHE_TTL_MS = 30 * 1e3;
    __name(getPlans, "getPlans");
  }
});

// src/handlers/price-history.ts
var price_history_exports = {};
__export(price_history_exports, {
  getPriceHistory: () => getPriceHistory
});
async function getPriceHistory(planIdParam) {
  try {
...
// src/handlers/address.ts
var address_exports = {};
__export(address_exports, {
  qualifyAddress: () => qualifyAddress,
...
async function qualifyAddress(request) {
  try {
    const url = new URL(request.url);
    ...
    const maxSpeeds = {
      "FTTP": 2e3,
      // Fiber to the Premises supports up to 2000/100 Mbps
      "FTTB": 100,
      "FTTC": 100,
      "FTTN": 100,
      "HFC": 2e3,
      // HFC (cable) now supports 2000/100 Mbps plans
      "WIRELESS": 50,
      "SATELLITE": 25
    };
    const qualification = {
...
var init_address = __esm({
  "src/handlers/address.ts"() {
    "use strict";
    init_cors();
    __name(searchAddress, "searchAddress");
    __name(qualifyAddress, "qualifyAddress");
  }
});
...
async function getStatus(_req, env) {
  try {
    ...
    const [
      ...
      db.prepare("SELECT COUNT(*) as count FROM providers WHERE active = 1").first(),
      db.prepare("SELECT COUNT(*) as count FROM providers WHERE active = 1 AND (needs_review = 1 OR last_error IS NOT NULL)").first(),
      db.prepare("SELECT COUNT(*) as count FROM providers WHERE active = 1 AND last_fetch_at > datetime('now', '-1 day') AND last_error IS NULL").first(),
      db.prepare("SELECT COUNT(*) as count FROM plans WHERE is_active = 1").first(),
      db.prepare("SELECT speed_tier, COUNT(*) as count FROM plans WHERE is_active = 1 AND speed_tier IS NOT NULL GROUP BY speed_tier ORDER BY speed_tier").all(),
      db.prepare("SELECT name, slug, last_error, last_fetch_at FROM providers WHERE last_error IS NOT NULL AND active = 1 ORDER BY last_fetch_at DESC LIMIT 5").all(),
      db.prepare("SELECT name, slug, last_fetch_at FROM providers WHERE active = 1 ORDER BY last_fetch_at ASC NULLS FIRST LIMIT 5").all(),
      db.prepare("SELECT COUNT(*) as count FROM plans WHERE is_active = 1 AND (source_url IS NULL OR source_url = '')").first(),
      db.prepare("SELECT COUNT(*) as count FROM plans WHERE is_active = 1 AND (ongoing_price_cents IS NULL OR ongoing_price_cents <= 0)").first(),
      db.prepare("SELECT COUNT(*) as count FROM plans WHERE is_active = 1 AND (speed_tier IS NULL OR speed_tier <= 0)").first()
    ]);
...
// src/handlers/admin-issues.ts
var admin_issues_exports = {};
__export(admin_issues_exports, {
  getIssues: () => getIssues
});
async function getIssues(_req) {
  const db = await getDb();
...
var init_admin_issues = __esm({
  "src/handlers/admin-issues.ts"() {
    "use strict";
    init_db();
    init_cors();
    __name(getIssues, "getIssues");
  }
});

// src/handlers/provider-verification.ts
var provider_verification_exports = {};
__export(provider_verification_exports, {
  getProviderVerification: () => getProviderVerification
});
async function getProviderVerification(req) {
  try {
    ...
    const withPercentages = metadata.map((m2) => ({
      ...m2,
      uploadSpeedPct: m2.plan_count > 0 ? (m2.has_upload_speed / m2.plan_count * 100).toFixed(0) : 0,
      dataAllowancePct: m2.plan_count > 0 ? (m2.has_data_allowance / m2.plan_count * 100).toFixed(0) : 0,
      contractMonthsPct: m2.plan_count > 0 ? (m2.has_contract_months / m2.plan_count * 100).toFixed(0) : 0,
      modemIncludedPct: m2.plan_count > 0 ? (m2.has_modem_included / m2.plan_count * 100).toFixed(0) : 0
    }));
...
var init_provider_verification = __esm({
  "src/handlers/provider-verification.ts"() {
    "use strict";
    init_db();
    __name(getProviderVerification, "getProviderVerification");
  }
});
...
var init_admin = __esm({
  "src/handlers/admin.ts"() {
    "use strict";
    init_db();
    __name(adminApprove, "adminApprove");
  }
});

// src/handlers/provider-comparison.ts
var provider_comparison_exports = {};
__export(provider_comparison_exports, {
  getProviderComparison: () => getProviderComparison
});
async function getProviderComparison(request, env) {
  try {
    const db = env.DB;
    const providers = await db.prepare(`
      ...
      ORDER BY name ASC
    `).all();
    const features = providers.results.map((p3) => ({
      id: p3.id,
      name: p3.name,
      slug: p3.slug,
      logo_url: p3.logo_url,
      features: {
        ipv6: {
          label: "IPv6 Support",
          value: p3.ipv6_support,
          display: p3.ipv6_support ? "\u2713" : "\u2717"
        },
        cgnat: {
          label: "CGNAT",
          value: p3.cgnat,
          display: p3.cgnat ? `Uses CGNAT${p3.cgnat_opt_out ? p3.cgnat_opt_out === 1 ? " (free opt-out)" : " (paid opt-out)" : ""}` : "No CGNAT"
        },
        static_ip: {
          label: "Static IP",
          value: p3.static_ip_available,
          display: p3.static_ip_available === 0 ? "\u2717" : p3.static_ip_available === 1 ? "Free" : "Paid addon"
        },
        australian_support: {
          label: "Support",
          value: p3.australian_support,
          display: p3.australian_support === 2 ? "100% Australian" : p3.australian_support === 1 ? "Mixed (AU+Offshore)" : "Offshore"
        },
        parent_company: {
          label: "Parent Company",
          value: p3.parent_company,
          display: p3.parent_company || "Independent"
        }
      },
      description: p3.description,
      support_hours: p3.support_hours
    }));
    return new Response(
...
var init_provider_comparison = __esm({
  "src/handlers/provider-comparison.ts"() {
    "use strict";
    __name(getProviderComparison, "getProviderComparison");
  }
});
    ...
    if (!result.allowed) {
      return new Response(JSON.stringify({
        ...
        headers: {
          ...
          "Retry-After": Math.ceil(((result.resetTime || Date.now()) - Date.now()) / 1e3).toString()
        }
      });
    }
...
// src/handlers/savings-calculator.ts
var savings_calculator_exports = {};
__export(savings_calculator_exports, {
  calculateSavings: () => calculateSavings
});
async function calculateSavings(request, env) {
  try {
    const { current_plan_id, proposed_plans, usage_gb_per_month, months = 12 } = await request.json();
    ...
    if (!Array.isArray(proposed_plans) || proposed_plans.length === 0 || proposed_plans.length > 20 || !proposed_plans.every((n2) => Number.isInteger(n2) && n2 > 0)) {
    ...
    if (typeof usage_gb_per_month !== "number" || usage_gb_per_month < 0 || usage_gb_per_month > 1e6) {
    ...
    try {
      const { createRateLimiter: createRateLimiter2 } = await Promise.resolve().then(() => (init_rate_limit(), rate_limit_exports));
      const limiter = createRateLimiter2({ windowMs: 6e4, maxRequests: 30 });
    ...
    const alternatives = (proposedResult.results || []).map((plan) => {
    ...
    const bestValue = alternatives.reduce(
      (best, alt) => best && alt.savings > best.savings ? alt : best || alt,
      alternatives[0] || null
    );
    return new Response(
      JSON.stringify({
        ...
        alternatives,
        total_potential_savings: Math.round(alternatives.reduce((sum, a2) => sum + a2.savings, 0) * 100) / 100,
...
var init_savings_calculator = __esm({
  "src/handlers/savings-calculator.ts"() {
    "use strict";
...
async function handleFeedback(request, env) {
  if (request.method === "POST") {
    try {
      const { createRateLimiter: createRateLimiter2 } = await Promise.resolve().then(() => (init_rate_limit(), rate_limit_exports));
      const limiter = createRateLimiter2({ windowMs: 6e4, maxRequests: 10 });
      const rate = await limiter(request);
      if (!rate.allowed) {
    ...
    if (feedback.description && typeof feedback.description === "string" && feedback.description.length > 1e3) {
    ...
    feedback.created_at = (/* @__PURE__ */ new Date()).toISOString();
    try {
      const db = env.DB;
...
__export(favicon_exports, {
  fetchProviderLogo: () => fetchProviderLogo,
  updateProviderFavicons: () => updateProviderFavicons
});
async function fetchProviderLogo(domain, _providerName) {
...
async function updateProviderFavicons() {
  const db = await getDb();
  const providersRes = await db.prepare("SELECT id, name, slug FROM providers").all();
  ...
  for (const provider of providers) {
    ...
    if (domain) {
      ...
      await db.prepare("UPDATE providers SET favicon_url = ? WHERE id = ?").bind(logoUrl, provider.id).run();
      updated++;
    }
  }
  return { updated };
...
var init_favicon = __esm({
  "src/lib/favicon.ts"() {
    "use strict";
    init_db();
    __name(fetchProviderLogo, "fetchProviderLogo");
    __name(updateProviderFavicons, "updateProviderFavicons");
  }
});

// src/handlers/data-verification.ts
var data_verification_exports = {};
__export(data_verification_exports, {
  verifyData: () => verifyData
});
async function verifyData(_req) {
  try {
    const db = await getDb();
    const providers = await db.prepare("SELECT COUNT(*) as count FROM providers WHERE active = 1").first();
    const plans = await db.prepare("SELECT COUNT(*) as count FROM plans WHERE is_active = 1").first();
    const plansWithIntro = await db.prepare("SELECT COUNT(*) as count FROM plans WHERE is_active = 1 AND intro_price_cents IS NOT NULL").first();
...
var init_data_verification = __esm({
  "src/handlers/data-verification.ts"() {
    "use strict";
    init_db();
    init_cors();
    __name(verifyData, "verifyData");
  }
});
...
// ../../packages/shared/dist/validators.js
function parsePriceToCents(text) {
  if (!text)
    return null;
  const clean = text.replace(/[ ,]/g, "");
  const m2 = clean.match(/(\d+(?:\.\d{1,2})?)/);
  if (!m2)
    return null;
...
function parseDurationToDays(text) {
  if (!text)
    return null;
  const m2 = text.match(/(\d+)\s*(month|months)/i);
  if (!m2)
    return null;
  return parseInt(m2[1]) * 30;
}
function extractPromoFromText(text) {
  if (!text)
    return { promoCode: null, promoDescription: null };
  const clean = text.replace(/\s+/g, " ").trim();
  if (!clean)
    return { promoCode: null, promoDescription: null };
  const codeMatch = clean.match(/\b(?:promo\s*code|code|use\s*code)\s*[:-]?\s*([A-Z0-9]{4,12})\b/i);
  const discountMatch = clean.match(/\$\s*\d+(?:\.\d{1,2})?\s*(?:off|discount|credit)/i) || clean.match(/\b(save|saving)\s*\$?\d+(?:\.\d{1,2})?\b/i);
  ...
  const parts = [];
  if (discountMatch)
    parts.push(discountMatch[0]);
  if (freeMatch)
    parts.push(freeMatch[0]);
  if (durationMatch)
    parts.push(`for ${durationMatch[1]} months`);
...
function extractIntroFromText(text) {
  if (!text)
    return { introPriceCents: null, introDurationDays: null };
  ...
  return {
    introPriceCents: introPriceCents >= 2e3 ? introPriceCents : null,
    introDurationDays
  };
}
function normalizeSpeed(text) {
  if (!text)
    return null;
  if (typeof text === "number") {
    const n2 = text;
    if ([12, 25, 50, 100, 200, 250, 400, 500, 1e3, 2e3].includes(n2))
      return n2;
    return null;
  }
  const s2 = String(text).trim().toLowerCase();
  const exact = s2.match(/(\d{1,4})/);
  if (exact) {
    const n2 = parseInt(exact[1]);
    if ([12, 25, 50, 100, 200, 250, 400, 500, 1e3, 2e3].includes(n2))
      return n2;
  }
...
function validatePlan(p3) {
  const errors = [];
  const warnings = [];
  const planName = (p3.planName || "").trim();
  const sourceUrl = (p3.sourceUrl || "").trim();
  if (!planName)
    errors.push("missing plan name");
  if (planName.length > 120)
    errors.push(`plan name too long (${planName.length})`);
  if (!p3.speedTier)
    errors.push("missing speed tier");
  if (!sourceUrl)
    errors.push("missing source url");
  const price = p3.ongoingPriceCents ?? p3.introPriceCents;
  if (price === null) {
    errors.push("missing price");
  } else {
    if (price < 2e3)
      errors.push(`price too low (${price})`);
  }
  ...
  if (p3.ongoingPriceCents !== null) {
    if (p3.ongoingPriceCents < 3e3)
      warnings.push(`ongoing_price_cents ${p3.ongoingPriceCents} unusually low`);
    if (p3.ongoingPriceCents > 3e4)
      warnings.push(`ongoing_price_cents ${p3.ongoingPriceCents} unusually high`);
  }
  if (p3.introPriceCents !== null && p3.ongoingPriceCents !== null) {
    if (p3.introPriceCents > p3.ongoingPriceCents)
      warnings.push("intro price higher than ongoing price");
  }
  if (p3.uploadSpeedMbps !== void 0 && p3.uploadSpeedMbps !== null && p3.speedTier !== null) {
    if (p3.uploadSpeedMbps > p3.speedTier)
      warnings.push("upload speed greater than download speed");
  }
...
function normalizeExtract(ex) {
  ...
  const normalizedIntroDuration = typeof ex.introDurationDays === "number" ? ex.introDurationDays : parseDurationToDays(String(ex.introDurationDays)) ?? null;
  const normalizedIntroPrice = typeof ex.introPriceCents === "number" ? ex.introPriceCents : parsePriceToCents(String(ex.introPriceCents)) ?? null;
  const normalizedOngoingPrice = typeof ex.ongoingPriceCents === "number" ? ex.ongoingPriceCents : parsePriceToCents(String(ex.ongoingPriceCents)) ?? null;
  ...
  const contractType = ex.contractType ?? (typeof ex.contractMonths === "number" ? ex.contractMonths === 0 ? "month-to-month" : `${ex.contractMonths}-month` : null);
  const serviceType = ex.serviceType ?? (ex.technologyType === "5g-home" ? "5g-home" : ex.technologyType === "satellite" ? "satellite" : "nbn");
  return {
    ...ex,
    introPriceCents,
...
var init_validators = __esm({
  "../../packages/shared/dist/validators.js"() {
    ...
    __name(parsePriceToCents, "parsePriceToCents");
    __name(parseDurationToDays, "parseDurationToDays");
    __name(extractPromoFromText, "extractPromoFromText");
    __name(extractIntroFromText, "extractIntroFromText");
    __name(normalizeSpeed, "normalizeSpeed");
    __name(validatePlan, "validatePlan");
    __name(normalizeExtract, "normalizeExtract");
  }
});

// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/shared/symbols.js
var require_symbols = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/shared/symbols.js"(exports) {
    "use strict";
...
var require_Parser = __commonJS({
  "../../node_modules/.pnpm/htmlparser2@10.1.0/node_modules/htmlparser2/dist/commonjs/Parser.js"(exports) {
    "use strict";
    var __createBinding = exports && exports.__createBinding || (Object.create ? (function(o3, m2, k, k2) {
      if (k2 === void 0) k2 = k;
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/shared/constants.js
var require_constants = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/shared/constants.js"(exports) {
    "use strict";
    var NODE_END = -1;
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/shared/object.js
var require_object = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/shared/object.js"(exports) {
    "use strict";
    var {
      assign,
      create,
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/shared/utils.js
var require_utils = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/shared/utils.js"(exports) {
    "use strict";
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/shared/shadow-roots.js
var require_shadow_roots = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/shared/shadow-roots.js"(exports) {
    "use strict";
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/custom-element-registry.js
var require_custom_element_registry = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/custom-element-registry.js"(exports) {
    "use strict";
    var { ELEMENT_NODE } = require_constants();
    ...
    var createTrigger = /* @__PURE__ */ __name((method, isConnected) => (element) => {
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/shared/parse-from-string.js
var require_parse_from_string = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/shared/parse-from-string.js"(exports) {
    "use strict";
    ...
    var notParsing = true;
    var append = /* @__PURE__ */ __name((self, node, active) => {
      const end = self[END];
    ...
    var isNotParsing = /* @__PURE__ */ __name(() => notParsing, "isNotParsing");
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/shared/register-html-class.js
var require_register_html_class = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/shared/register-html-class.js"(exports) {
    "use strict";
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/shared/jsdon.js
var require_jsdon = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/shared/jsdon.js"(exports) {
    "use strict";
    var {
      NODE_END,
      ATTRIBUTE_NODE,
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/mutation-observer.js
var require_mutation_observer = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/mutation-observer.js"(exports) {
    "use strict";
    ...
    var queueAttribute = /* @__PURE__ */ __name((observer, target, attributeName, attributeFilter, attributeOldValue, oldValue) => {
      if (!attributeFilter || attributeFilter.includes(attributeName)) {
        const { callback, records, scheduled: scheduled2 } = observer;
    ...
    var attributeChangedCallback = /* @__PURE__ */ __name((element, attributeName, oldValue) => {
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/shared/attributes.js
var require_attributes = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/shared/attributes.js"(exports) {
    "use strict";
    ...
    var setAttribute = /* @__PURE__ */ __name((element, attribute) => {
    ...
    var removeAttribute = /* @__PURE__ */ __name((element, attribute) => {
      const { [VALUE]: value, name } = attribute;
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/event-target.js
var require_event_target = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/event-target.js"(exports) {
    "use strict";
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/node-list.js
var require_node_list = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/node-list.js"(exports) {
    "use strict";
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/node.js
var require_node2 = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/node.js"(exports) {
    "use strict";
    var {
      ELEMENT_NODE,
      ATTRIBUTE_NODE,
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/shared/text-escaper.js
var require_text_escaper = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/shared/text-escaper.js"(exports) {
    "use strict";
    var { replace } = "";
    var ca = /[<>&\xA0]/g;
    var esca = {
    ...
    var pe = /* @__PURE__ */ __name((m2) => esca[m2], "pe");
    var escape = /* @__PURE__ */ __name((es) => replace.call(es, ca, pe), "escape");
    exports.escape = escape;
  }
});

// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/attr.js
var require_attr = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/attr.js"(exports) {
    "use strict";
    ...
    var Attr = class _Attr extends Node {
      ...
      toString() {
        ...
        const escapedValue2 = (ignoreCase(this) ? value : escape(value)).replace(QUOTE, "&quot;");
        return `${name}="${escapedValue2}"`;
      }
      toJSON() {
        const json = [];
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/shared/node.js
var require_node3 = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/shared/node.js"(exports) {
    "use strict";
    var {
      CDATA_SECTION_NODE,
      COMMENT_NODE,
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/mixin/non-document-type-child-node.js
var require_non_document_type_child_node = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/mixin/non-document-type-child-node.js"(exports) {
    "use strict";
    var { ELEMENT_NODE } = require_constants();
    var { nextSibling, previousSibling } = require_node3();
    var nextElementSibling = /* @__PURE__ */ __name((node) => {
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/mixin/child-node.js
var require_child_node = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/mixin/child-node.js"(exports) {
    "use strict";
    var { ELEMENT_NODE } = require_constants();
    var { NEXT, PREV } = require_symbols();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/character-data.js
var require_character_data = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/character-data.js"(exports) {
    "use strict";
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/cdata-section.js
var require_cdata_section = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/cdata-section.js"(exports) {
    "use strict";
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/comment.js
var require_comment = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/comment.js"(exports) {
    "use strict";
...
var require_commonjs2 = __commonJS({
  "../../node_modules/.pnpm/css-what@6.2.2/node_modules/css-what/lib/commonjs/index.js"(exports) {
    ...
    Object.defineProperty(exports, "isTraversal", { enumerable: true, get: /* @__PURE__ */ __name(function() {
...
var require_attributes2 = __commonJS({
  "../../node_modules/.pnpm/css-select@5.2.2/node_modules/css-select/lib/attributes.js"(exports) {
    ...
    exports.attributeRules = {
      equals: /* @__PURE__ */ __name(function(next, data, options) {
      ...
      hyphen: /* @__PURE__ */ __name(function(next, data, options) {
        var adapter = options.adapter;
        var name = data.name;
        var value = data.value;
      ...
      element: /* @__PURE__ */ __name(function(next, data, options) {
        var adapter = options.adapter;
      ...
      exists: /* @__PURE__ */ __name(function(next, _a, _b) {
        var name = _a.name;
      ...
      start: /* @__PURE__ */ __name(function(next, data, options) {
        var adapter = options.adapter;
        var name = data.name;
        var value = data.value;
        var len = value.length;
      ...
      end: /* @__PURE__ */ __name(function(next, data, options) {
        var adapter = options.adapter;
        var name = data.name;
        var value = data.value;
        var len = -value.length;
      ...
      any: /* @__PURE__ */ __name(function(next, data, options) {
        var adapter = options.adapter;
        var name = data.name, value = data.value;
        if (value === "") {
          return boolbase_1.default.falseFunc;
      ...
      not: /* @__PURE__ */ __name(function(next, data, options) {
        var adapter = options.adapter;
        var name = data.name;
        var value = data.value;
        if (value === "") {
...
var require_lib6 = __commonJS({
  "../../node_modules/.pnpm/nth-check@2.1.1/node_modules/nth-check/lib/index.js"(exports) {
    ...
    Object.defineProperty(exports, "compile", { enumerable: true, get: /* @__PURE__ */ __name(function() {
...
var require_filters = __commonJS({
  "../../node_modules/.pnpm/css-select@5.2.2/node_modules/css-select/lib/pseudo-selectors/filters.js"(exports) {
    ...
    exports.filters = {
      contains: /* @__PURE__ */ __name(function(next, text, _a) {
        var adapter = _a.adapter;
        return /* @__PURE__ */ __name(function contains(elem) {
          return next(elem) && adapter.getText(elem).includes(text);
        }, "contains");
      }, "contains"),
      icontains: /* @__PURE__ */ __name(function(next, text, _a) {
        var adapter = _a.adapter;
      ...
      "nth-child": /* @__PURE__ */ __name(function(next, rule, _a) {
      ...
      "nth-last-child": /* @__PURE__ */ __name(function(next, rule, _a) {
        var adapter = _a.adapter, equals = _a.equals;
        var func = (0, nth_check_1.default)(rule);
        if (func === boolbase_1.default.falseFunc)
          return boolbase_1.default.falseFunc;
      ...
      "nth-of-type": /* @__PURE__ */ __name(function(next, rule, _a) {
        var adapter = _a.adapter, equals = _a.equals;
        var func = (0, nth_check_1.default)(rule);
        if (func === boolbase_1.default.falseFunc)
          return boolbase_1.default.falseFunc;
      ...
      "nth-last-of-type": /* @__PURE__ */ __name(function(next, rule, _a) {
        var adapter = _a.adapter, equals = _a.equals;
        var func = (0, nth_check_1.default)(rule);
        if (func === boolbase_1.default.falseFunc)
          return boolbase_1.default.falseFunc;
      ...
      root: /* @__PURE__ */ __name(function(next, _rule, _a) {
        var adapter = _a.adapter;
        return function(elem) {
          var parent = adapter.getParent(elem);
          return (parent == null || !adapter.isTag(parent)) && next(elem);
      ...
      scope: /* @__PURE__ */ __name(function(next, rule, options, context) {
    ...
    function dynamicStatePseudo(name) {
      return /* @__PURE__ */ __name(function dynamicPseudo(next, _rule, _a) {
        var adapter = _a.adapter;
...
var require_subselects = __commonJS({
  "../../node_modules/.pnpm/css-select@5.2.2/node_modules/css-select/lib/pseudo-selectors/subselects.js"(exports) {
    ...
    var is = /* @__PURE__ */ __name(function(next, token, options, context, compileToken) {
    ...
    exports.subselects = {
      ...
      matches: is,
      where: is,
      not: /* @__PURE__ */ __name(function(next, token, options, context, compileToken) {
        var func = compileToken(token, copyOptions(options), context);
        return func === boolbase_1.default.falseFunc ? next : func === boolbase_1.default.trueFunc ? boolbase_1.default.falseFunc : function(elem) {
          return !func(elem) && next(elem);
        };
      }, "not"),
      has: /* @__PURE__ */ __name(function(next, subselect, options, _context, compileToken) {
        var adapter = options.adapter;
...
var require_pseudo_selectors = __commonJS({
  "../../node_modules/.pnpm/css-select@5.2.2/node_modules/css-select/lib/pseudo-selectors/index.js"(exports) {
    ...
    Object.defineProperty(exports, "filters", { enumerable: true, get: /* @__PURE__ */ __name(function() {
      return filters_js_1.filters;
    }, "get") });
    var pseudos_js_1 = require_pseudos();
    Object.defineProperty(exports, "pseudos", { enumerable: true, get: /* @__PURE__ */ __name(function() {
      return pseudos_js_1.pseudos;
    }, "get") });
    var aliases_js_1 = require_aliases();
    Object.defineProperty(exports, "aliases", { enumerable: true, get: /* @__PURE__ */ __name(function() {
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/shared/matches.js
var require_matches = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/shared/matches.js"(exports) {
    "use strict";
    var CSSselect = require_lib7();
    var { ELEMENT_NODE, TEXT_NODE } = require_constants();
    var { ignoreCase } = require_utils();
    var { isArray: isArray2 } = Array;
    var isTag = /* @__PURE__ */ __name(({ nodeType }) => nodeType === ELEMENT_NODE, "isTag");
    var existsOne = /* @__PURE__ */ __name((test, elements) => elements.some(
      (element) => isTag(element) && (test(element) || existsOne(test, getChildren(element)))
    ), "existsOne");
    var getAttributeValue = /* @__PURE__ */ __name((element, name) => name === "class" ? element.classList.value : element.getAttribute(name), "getAttributeValue");
    var getChildren = /* @__PURE__ */ __name(({ childNodes }) => childNodes, "getChildren");
    var getName = /* @__PURE__ */ __name((element) => {
      const { localName } = element;
      return ignoreCase(element) ? localName.toLowerCase() : localName;
    }, "getName");
    var getParent = /* @__PURE__ */ __name(({ parentNode }) => parentNode, "getParent");
    var getSiblings = /* @__PURE__ */ __name((element) => {
      const { parentNode } = element;
      return parentNode ? getChildren(parentNode) : element;
    }, "getSiblings");
    var getText = /* @__PURE__ */ __name((node) => {
      if (isArray2(node))
        return node.map(getText).join("");
      if (isTag(node))
    ...
    var hasAttrib = /* @__PURE__ */ __name((element, name) => element.hasAttribute(name), "hasAttrib");
    var removeSubsets = /* @__PURE__ */ __name((nodes) => {
      let { length } = nodes;
      while (length--) {
    ...
    var findAll = /* @__PURE__ */ __name((test, nodes) => {
      const matches2 = [];
    ...
    var findOne = /* @__PURE__ */ __name((test, nodes) => {
    ...
    var prepareMatch = /* @__PURE__ */ __name((element, selectors) => CSSselect.compile(
      selectors,
      {
    ...
    var matches = /* @__PURE__ */ __name((element, selectors) => CSSselect.is(
      element,
      selectors,
      {
        strict: true,
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/text.js
var require_text = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/text.js"(exports) {
    "use strict";
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/mixin/parent-node.js
var require_parent_node = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/mixin/parent-node.js"(exports) {
    "use strict";
    var {
      ATTRIBUTE_NODE,
      DOCUMENT_FRAGMENT_NODE,
    ...
    var isNode = /* @__PURE__ */ __name((node) => node instanceof Node, "isNode");
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/mixin/non-element-parent-node.js
var require_non_element_parent_node = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/mixin/non-element-parent-node.js"(exports) {
    "use strict";
    var { ELEMENT_NODE } = require_constants();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/document-fragment.js
var require_document_fragment = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/document-fragment.js"(exports) {
    "use strict";
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/document-type.js
var require_document_type = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/document-type.js"(exports) {
    "use strict";
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/mixin/inner-html.js
var require_inner_html = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/mixin/inner-html.js"(exports) {
    "use strict";
...
// ../../node_modules/.pnpm/uhyphen@0.2.0/node_modules/uhyphen/cjs/index.js
var require_cjs = __commonJS({
  "../../node_modules/.pnpm/uhyphen@0.2.0/node_modules/uhyphen/cjs/index.js"(exports, module) {
    "use strict";
    module.exports = (camel) => camel.replace(/(([A-Z0-9])([A-Z0-9][a-z]))|(([a-z0-9]+)([A-Z]))/g, "$2$5-$3$6").toLowerCase();
  }
});

// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/dom/string-map.js
var require_string_map = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/dom/string-map.js"(exports) {
    "use strict";
    var uhyphen = ((m2) => (
      /* c8 ignore start */
      m2.__esModule ? m2.default : m2
    ))(require_cjs());
    var { setPrototypeOf } = require_object();
    var refs = /* @__PURE__ */ new WeakMap();
    var key = /* @__PURE__ */ __name((name) => `data-${uhyphen(name)}`, "key");
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/dom/token-list.js
var require_token_list = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/dom/token-list.js"(exports) {
    "use strict";
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/css-style-declaration.js
var require_css_style_declaration = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/css-style-declaration.js"(exports) {
    "use strict";
    var uhyphen = ((m2) => (
      /* c8 ignore start */
      m2.__esModule ? m2.default : m2
    ...
    var refs = /* @__PURE__ */ new WeakMap();
    var getKeys = /* @__PURE__ */ __name((style) => [...style.keys()].filter((key) => key !== PRIVATE), "getKeys");
    ...
    var CSSStyleDeclaration = class extends Map {
      ...
      [Symbol.iterator]() {
        ...
        return {
          next() {
            ...
            return { done, value: done ? null : keys[i2++] };
          }
        };
      }
      get [PRIVATE]() {
      ...
      toString() {
        ...
        updateKeys(self);
        const cssText = [];
        self.forEach(push2, cssText);
        return cssText.join(";");
      }
    };
    exports.CSSStyleDeclaration = CSSStyleDeclaration;
    var { prototype } = CSSStyleDeclaration;
    function push2(value, key) {
      if (key !== PRIVATE)
        this.push(`${key}:${value}`);
    }
    __name(push2, "push");
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/event.js
var require_event = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/event.js"(exports) {
    "use strict";
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/named-node-map.js
var require_named_node_map = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/named-node-map.js"(exports) {
    "use strict";
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/shadow-root.js
var require_shadow_root = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/shadow-root.js"(exports) {
    "use strict";
    var { DOCUMENT_FRAGMENT_NODE } = require_constants();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/element.js
var require_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/element.js"(exports) {
    "use strict";
    var {
      ATTRIBUTE_NODE,
      BLOCK_ELEMENTS,
    ...
    var create = /* @__PURE__ */ __name((ownerDocument, element, localName) => {
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/svg/element.js
var require_element2 = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/svg/element.js"(exports) {
    "use strict";
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/shared/facades.js
var require_facades = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/shared/facades.js"(exports) {
    "use strict";
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/element.js
var require_element3 = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/element.js"(exports) {
    "use strict";
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/template-element.js
var require_template_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/template-element.js"(exports) {
    "use strict";
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/html-element.js
var require_html_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/html-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/text-element.js
var require_text_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/text-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/script-element.js
var require_script_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/script-element.js"(exports) {
    "use strict";
    var { booleanAttribute, stringAttribute } = require_attributes();
    var { registerHTMLClass } = require_register_html_class();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/frame-element.js
var require_frame_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/frame-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/i-frame-element.js
var require_i_frame_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/i-frame-element.js"(exports) {
    "use strict";
    var { registerHTMLClass } = require_register_html_class();
    var { booleanAttribute, stringAttribute } = require_attributes();
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/object-element.js
var require_object_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/object-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/head-element.js
var require_head_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/head-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/body-element.js
var require_body_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/body-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
var require_parse3 = __commonJS({
  "../../node_modules/.pnpm/cssom@0.5.0/node_modules/cssom/lib/parse.js"(exports) {
    var CSSOM = {};
    CSSOM.parse = /* @__PURE__ */ __name(function parse24(token) {
      var i2 = 0;
...
var require_CSSStyleDeclaration = __commonJS({
  "../../node_modules/.pnpm/cssom@0.5.0/node_modules/cssom/lib/CSSStyleDeclaration.js"(exports) {
    var CSSOM = {};
    CSSOM.CSSStyleDeclaration = /* @__PURE__ */ __name(function CSSStyleDeclaration() {
      this.length = 0;
      this.parentRule = null;
      this._importants = {};
    }, "CSSStyleDeclaration");
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/style-element.js
var require_style_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/style-element.js"(exports) {
    "use strict";
    var { parse: parse24 } = require_lib8();
    var { registerHTMLClass } = require_register_html_class();
    ...
    var HTMLStyleElement = class extends TextElement {
      ...
      get sheet() {
        ...
        return this[SHEET] = parse24(this.textContent);
      }
      get innerHTML() {
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/time-element.js
var require_time_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/time-element.js"(exports) {
    "use strict";
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/field-set-element.js
var require_field_set_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/field-set-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/embed-element.js
var require_embed_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/embed-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/hr-element.js
var require_hr_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/hr-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/progress-element.js
var require_progress_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/progress-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/paragraph-element.js
var require_paragraph_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/paragraph-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/table-element.js
var require_table_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/table-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/frame-set-element.js
var require_frame_set_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/frame-set-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/li-element.js
var require_li_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/li-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/base-element.js
var require_base_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/base-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/data-list-element.js
var require_data_list_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/data-list-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/input-element.js
var require_input_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/input-element.js"(exports) {
    "use strict";
    var { registerHTMLClass } = require_register_html_class();
    var { booleanAttribute, stringAttribute } = require_attributes();
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/param-element.js
var require_param_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/param-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/media-element.js
var require_media_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/media-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/audio-element.js
var require_audio_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/audio-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/heading-element.js
var require_heading_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/heading-element.js"(exports) {
    "use strict";
    var { registerHTMLClass } = require_register_html_class();
    var { HTMLElement } = require_element3();
    var tagName = "h1";
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/directory-element.js
var require_directory_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/directory-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/quote-element.js
var require_quote_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/quote-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
var require_canvas_shim = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/commonjs/canvas-shim.cjs"(exports, module) {
    ...
    module.exports = {
      createCanvas: /* @__PURE__ */ __name((width, height) => new Canvas(width, height), "createCanvas")
    };
  }
});
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/canvas-element.js
var require_canvas_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/canvas-element.js"(exports) {
    "use strict";
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/legend-element.js
var require_legend_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/legend-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/option-element.js
var require_option_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/option-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
    var { booleanAttribute, stringAttribute } = require_attributes();
    var { registerHTMLClass } = require_register_html_class();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/span-element.js
var require_span_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/span-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/meter-element.js
var require_meter_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/meter-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/video-element.js
var require_video_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/video-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/table-cell-element.js
var require_table_cell_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/table-cell-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/title-element.js
var require_title_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/title-element.js"(exports) {
    "use strict";
    var { registerHTMLClass } = require_register_html_class();
    var { TextElement } = require_text_element();
    var tagName = "title";
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/output-element.js
var require_output_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/output-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/table-row-element.js
var require_table_row_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/table-row-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/data-element.js
var require_data_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/data-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/menu-element.js
var require_menu_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/menu-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/select-element.js
var require_select_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/select-element.js"(exports) {
    "use strict";
    var { registerHTMLClass } = require_register_html_class();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/br-element.js
var require_br_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/br-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/button-element.js
var require_button_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/button-element.js"(exports) {
    "use strict";
    var { registerHTMLClass } = require_register_html_class();
    var { booleanAttribute } = require_attributes();
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/map-element.js
var require_map_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/map-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/opt-group-element.js
var require_opt_group_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/opt-group-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/d-list-element.js
var require_d_list_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/d-list-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/text-area-element.js
var require_text_area_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/text-area-element.js"(exports) {
    "use strict";
    var { registerHTMLClass } = require_register_html_class();
    var { booleanAttribute } = require_attributes();
    var { TextElement } = require_text_element();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/font-element.js
var require_font_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/font-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/div-element.js
var require_div_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/div-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/link-element.js
var require_link_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/link-element.js"(exports) {
    "use strict";
    var { registerHTMLClass } = require_register_html_class();
    var { booleanAttribute, stringAttribute } = require_attributes();
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/slot-element.js
var require_slot_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/slot-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
    var { registerHTMLClass } = require_register_html_class();
    var tagName = "slot";
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/form-element.js
var require_form_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/form-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/image-element.js
var require_image_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/image-element.js"(exports) {
    "use strict";
    var { registerHTMLClass } = require_register_html_class();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/pre-element.js
var require_pre_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/pre-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/u-list-element.js
var require_u_list_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/u-list-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/meta-element.js
var require_meta_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/meta-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
    var { registerHTMLClass } = require_register_html_class();
    var { stringAttribute } = require_attributes();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/picture-element.js
var require_picture_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/picture-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/area-element.js
var require_area_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/area-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/o-list-element.js
var require_o_list_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/o-list-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/table-caption-element.js
var require_table_caption_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/table-caption-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/anchor-element.js
var require_anchor_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/anchor-element.js"(exports) {
    "use strict";
    var { registerHTMLClass } = require_register_html_class();
    var { stringAttribute } = require_attributes();
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/label-element.js
var require_label_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/label-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/unknown-element.js
var require_unknown_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/unknown-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/mod-element.js
var require_mod_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/mod-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/details-element.js
var require_details_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/details-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/source-element.js
var require_source_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/source-element.js"(exports) {
    "use strict";
    var { registerHTMLClass } = require_register_html_class();
    var { stringAttribute } = require_attributes();
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/track-element.js
var require_track_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/track-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/marquee-element.js
var require_marquee_element = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/marquee-element.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/shared/html-classes.js
var require_html_classes = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/shared/html-classes.js"(exports) {
    "use strict";
    var { HTMLElement } = require_element3();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/shared/mime.js
var require_mime = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/shared/mime.js"(exports) {
    "use strict";
    var voidElements = { test: /* @__PURE__ */ __name(() => true, "test") };
    var Mime = {
      "text/html": {
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/custom-event.js
var require_custom_event = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/custom-event.js"(exports) {
    "use strict";
    var { Event } = require_event();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/input-event.js
var require_input_event = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/input-event.js"(exports) {
    "use strict";
    var { Event } = require_event();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/image.js
var require_image = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/image.js"(exports) {
    "use strict";
    var { HTMLImageElement } = require_image_element();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/range.js
var require_range = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/range.js"(exports) {
    "use strict";
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/tree-walker.js
var require_tree_walker = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/tree-walker.js"(exports) {
    "use strict";
    var {
      DOCUMENT_NODE,
      ELEMENT_NODE,
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/document.js
var require_document = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/document.js"(exports) {
    "use strict";
      ...
      cloneNode(deep = false) {
        ...
        if (deep) {
          ...
          for (let { length } = childNodes, i2 = 0; i2 < length; i2++)
            document.insertBefore(childNodes[i2].cloneNode(true), end);
          if (doctype)
    ...
    exports.Document = Document;
    setPrototypeOf(
      globalExports.Document = /* @__PURE__ */ __name(function Document2() {
        illegalConstructor();
      }, "Document"),
      Document
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/document.js
var require_document2 = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/html/document.js"(exports) {
    "use strict";
    var { ELEMENT_NODE } = require_constants();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/svg/document.js
var require_document3 = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/svg/document.js"(exports) {
    "use strict";
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/xml/document.js
var require_document4 = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/xml/document.js"(exports) {
    "use strict";
    var { MIME } = require_symbols();
    var { Document } = require_document();
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/dom/parser.js
var require_parser = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/dom/parser.js"(exports) {
    "use strict";
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/shared/parse-json.js
var require_parse_json = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/shared/parse-json.js"(exports) {
    "use strict";
    var {
      NODE_END,
      ELEMENT_NODE,
    ...
    var { HTMLElement } = require_element3();
    var { SVGElement } = require_element2();
    var { parse: parse24 } = JSON;
    var append = /* @__PURE__ */ __name((parentNode, node, end) => {
      node.parentNode = parentNode;
      knownSiblings(end[PREV], node, end);
    }, "append");
    var createHTMLElement = /* @__PURE__ */ __name((ownerDocument, localName) => {
    ...
    var parseJSON = /* @__PURE__ */ __name((value) => {
      const array = typeof value === "string" ? parse24(value) : value;
      const { length } = array;
      const document = new HTMLDocument();
      let parentNode = document, end = parentNode[END], svg = false, i2 = 0;
      while (i2 < length) {
        let nodeType = array[i2++];
        switch (nodeType) {
          case ELEMENT_NODE: {
            const localName = array[i2++];
          ...
          case ATTRIBUTE_NODE: {
            const name = array[i2++];
            const value2 = typeof array[i2] === "string" ? array[i2++] : "";
          ...
          case TEXT_NODE:
            append(parentNode, new Text(document, array[i2++]), end);
            break;
          case COMMENT_NODE:
            append(parentNode, new Comment(document, array[i2++]), end);
            break;
          case CDATA_SECTION_NODE:
            append(parentNode, new CDATASection(document, array[i2++]), end);
            break;
          case DOCUMENT_TYPE_NODE: {
            const args = [document];
            while (typeof array[i2] === "string")
              args.push(array[i2++]);
    ...
    var toJSON = /* @__PURE__ */ __name((node) => node.toJSON(), "toJSON");
    exports.toJSON = toJSON;
  }
});

// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/node-filter.js
var require_node_filter = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/interface/node-filter.js"(exports) {
    "use strict";
    var {
      SHOW_ALL,
      SHOW_ELEMENT,
...
// ../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/index.js
var require_cjs2 = __commonJS({
  "../../node_modules/.pnpm/linkedom@0.18.12/node_modules/linkedom/cjs/index.js"(exports) {
    "use strict";
    ...
    var { setPrototypeOf } = require_object();
    ((m2) => {
      exports.parseJSON = m2.parseJSON;
      exports.toJSON = m2.toJSON;
    })(require_parse_json());
    ((m2) => Object.keys(m2).map((k) => k !== "default" && (exports[k] = m2[k])))(require_facades());
    ((m2) => Object.keys(m2).map((k) => k !== "default" && (exports[k] = m2[k])))(require_html_classes());
    exports.DOMParser = DOMParser;
    ((m2) => {
      exports.CustomEvent = m2.CustomEvent;
    })(require_custom_event());
    ((m2) => {
      exports.Event = m2.Event;
    })(require_event());
    ((m2) => {
      exports.EventTarget = m2.EventTarget;
    })(require_event_target());
    ((m2) => {
      exports.InputEvent = m2.InputEvent;
    })(require_input_event());
    ((m2) => {
      exports.NodeList = m2.NodeList;
    })(require_node_list());
    ((m2) => {
      exports.NodeFilter = m2.NodeFilter;
    })(require_node_filter());
    var parseHTML2 = /* @__PURE__ */ __name((html, globals = null) => new DOMParser().parseFromString(
      html,
      "text/html",
      globals
...
// ../../packages/shared/dist/parsers/dom-utils.js
function getDOMParser() {
  if (DOMParserCtorImpl) {
    return new DOMParserCtorImpl();
  }
  if (typeof globalThis.DOMParser !== "undefined") {
    DOMParserCtorImpl = globalThis.DOMParser;
    return new DOMParserCtorImpl();
  }
  try {
    const { DOMParser } = require_cjs2();
    DOMParserCtorImpl = DOMParser;
    return new DOMParserCtorImpl();
  } catch {
...
var DOMParserCtorImpl;
var init_dom_utils = __esm({
  "../../packages/shared/dist/parsers/dom-utils.js"() {
    "use strict";
    DOMParserCtorImpl = null;
    __name(getDOMParser, "getDOMParser");
    __name(parseHTML, "parseHTML");
  }
});

// ../../packages/shared/dist/parsers/providers/aussiebroadband.js
var aussiebroadband_exports = {};
__export(aussiebroadband_exports, {
  canHandle: () => canHandle,
  parse: () => parse
});
...
function extractUploadSpeed(planElement, downloadSpeed) {
  if (!downloadSpeed)
    return null;
...
function extractDataAllowance(planElement) {
...
function extractContractMonths(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
...
function extractModemIncluded(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("modem included") || planText.includes("router included") || planText.includes("free modem")) {
    return true;
  }
  if (planText.includes("modem $") || planText.includes("bring your own") || planText.includes("your own modem")) {
    return false;
  }
  return null;
}
function extractSetupFee(planElement) {
  const planText = planElement.textContent || "";
...
var init_aussiebroadband = __esm({
  "../../packages/shared/dist/parsers/providers/aussiebroadband.js"() {
    "use strict";
    init_validators();
    init_dom_utils();
    __name(canHandle, "canHandle");
    __name(extractUploadSpeed, "extractUploadSpeed");
    __name(extractDataAllowance, "extractDataAllowance");
    __name(extractContractMonths, "extractContractMonths");
    __name(extractModemIncluded, "extractModemIncluded");
    __name(extractSetupFee, "extractSetupFee");
    __name(extractIntroOffer, "extractIntroOffer");
    __name(parse, "parse");
  }
});

// ../../packages/shared/dist/parsers/providers/spintel.js
var spintel_exports = {};
__export(spintel_exports, {
  canHandle: () => canHandle2,
  parse: () => parse2
});
function extractUploadSpeed2(planElement, downloadSpeed) {
  if (!downloadSpeed)
    return null;
  const planText = planElement.textContent || "";
  const uploadMatch = planText.match(/(\d+)\s*Mbps\s*(up|upload|↑)/i);
...
function extractDataAllowance2(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("unlimited"))
    return "Unlimited";
  const amountMatch = planText.match(/(\d+\.?\d*)\s*(tb|gb)/i);
...
function extractContractMonths2(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("no contract") || planText.includes("month-to-month"))
    return 0;
  if (planText.includes("24 month") || planText.includes("2 year"))
...
function extractModemIncluded2(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("modem included") || planText.includes("router included") || planText.includes("free modem"))
    return true;
  if (planText.includes("modem $") || planText.includes("bring your own") || planText.includes("your own modem"))
...
function extractSetupFee2(planElement) {
  const planText = planElement.textContent || "";
  if (planText.match(/free\s*(setup|connection|installation)/i))
    return 0;
  const feeMatch = planText.match(/(setup|connection|installation)[\s:]*\$(\d+(?:\.\d{2})?)/i);
...
function extractIntroOffer2(text) {
  const introMatch = text.match(/\$\s*([0-9]+(?:\.[0-9]{1,2})?)\s*(?:\/mo|\/month|per\s*month|pm)?\s*(?:for|first)\s*(\d{1,2})\s*(?:months|month|mo)/i);
  const thenMatch = text.match(/then\s*\$\s*([0-9]+(?:\.[0-9]{1,2})?)/i);
  const introPriceCents = introMatch ? parsePriceToCents(introMatch[1]) : null;
  const introDurationDays = introMatch ? parseInt(introMatch[2], 10) * 30 : null;
...
async function parse2(html, url) {
  ...
  if (rows.length === 0) {
    ...
    for (const p3 of priceEls) {
      const speedTier = null;
      const introOffer = extractIntroOffer2(p3.textContent || "");
      results.push({
        providerSlug: "spintel",
        planName: p3.previousElementSibling?.textContent?.trim() || "Plan",
        speedTier,
        uploadSpeedMbps: extractUploadSpeed2(p3, speedTier),
        dataAllowance: extractDataAllowance2(p3),
        contractMonths: extractContractMonths2(p3),
        modemIncluded: extractModemIncluded2(p3),
        introPriceCents: introOffer.introPriceCents ?? parsePriceToCents(p3.textContent || ""),
        introDurationDays: introOffer.introDurationDays,
        ongoingPriceCents: introOffer.thenPriceCents ?? parsePriceToCents(p3.textContent || ""),
        minTermDays: null,
        setupFeeCents: extractSetupFee2(p3),
        modemCostCents: null,
        conditionsText: (p3.textContent || "").trim() || null,
        promoDescription: introOffer.promoDescription,
        typicalEveningSpeedMbps: null,
        sourceUrl: url
      });
    ...
    for (const el of rows) {
      const introOffer = extractIntroOffer2(el.textContent || "");
      const s2 = (el.textContent || "").match(/NBN\s*(\d{1,4})/i)?.[1] ?? null;
      const parsedSpeed = s2 ? parseInt(s2) : null;
      ...
      results.push({
        ...
        speedTier,
        uploadSpeedMbps: extractUploadSpeed2(el, speedTier),
        dataAllowance: extractDataAllowance2(el),
        contractMonths: extractContractMonths2(el),
        modemIncluded: extractModemIncluded2(el),
        introPriceCents: introOffer.introPriceCents ?? parsePriceToCents(el.querySelector(".price")?.textContent || ""),
        introDurationDays: introOffer.introDurationDays,
        ongoingPriceCents: introOffer.thenPriceCents ?? parsePriceToCents(el.querySelector(".price")?.textContent || ""),
        minTermDays: null,
        setupFeeCents: extractSetupFee2(el),
        modemCostCents: null,
        conditionsText: (el.textContent || "").trim() || null,
        promoDescription: introOffer.promoDescription,
        typicalEveningSpeedMbps: null,
...
var init_spintel = __esm({
  "../../packages/shared/dist/parsers/providers/spintel.js"() {
    "use strict";
    init_validators();
    init_dom_utils();
    __name(extractUploadSpeed2, "extractUploadSpeed");
    __name(extractDataAllowance2, "extractDataAllowance");
    __name(extractContractMonths2, "extractContractMonths");
    __name(extractModemIncluded2, "extractModemIncluded");
    __name(extractSetupFee2, "extractSetupFee");
    __name(extractIntroOffer2, "extractIntroOffer");
    __name(canHandle2, "canHandle");
    __name(parse2, "parse");
  }
});

// ../../packages/shared/dist/parsers/providers/dodo.js
var dodo_exports = {};
__export(dodo_exports, {
  canHandle: () => canHandle3,
  parse: () => parse3
});
function extractUploadSpeed3(planElement, downloadSpeed) {
  if (!downloadSpeed)
    return null;
  const planText = planElement.textContent || "";
  const uploadMatch = planText.match(/(\d+)\s*Mbps\s*(up|upload|↑)/i);
...
function extractDataAllowance3(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("unlimited"))
    return "Unlimited";
  const amountMatch = planText.match(/(\d+\.?\d*)\s*(tb|gb)/i);
...
function extractContractMonths3(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("no contract") || planText.includes("month-to-month"))
    return 0;
  if (planText.includes("24 month") || planText.includes("2 year"))
...
function extractModemIncluded3(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("modem included") || planText.includes("router included") || planText.includes("free modem"))
    return true;
  if (planText.includes("modem $") || planText.includes("bring your own") || planText.includes("your own modem"))
...
function extractSetupFee3(planElement) {
  const planText = planElement.textContent || "";
  if (planText.match(/free\s*(setup|connection|installation)/i))
    return 0;
  const feeMatch = planText.match(/(setup|connection|installation)[\s:]*\$(\d+(?:\.\d{2})?)/i);
...
function extractIntroOffer3(text) {
  const introMatch = text.match(/\$\s*([0-9]+(?:\.[0-9]{1,2})?)\s*(?:\/mo|\/month|per\s*month|pm)?\s*(?:for|first)\s*(\d{1,2})\s*(?:months|month|mo)/i);
  const thenMatch = text.match(/then\s*\$\s*([0-9]+(?:\.[0-9]{1,2})?)/i);
  const introPriceCents = introMatch ? parsePriceToCents(introMatch[1]) : null;
  const introDurationDays = introMatch ? parseInt(introMatch[2], 10) * 30 : null;
...
function canHandle3(url) {
  return url.includes("dodo.com.au");
}
async function parse3(html, url) {
  const doc = parseHTML(html);
  const plans = Array.from(doc.querySelectorAll(".plan, .product"));
  return plans.map((p3) => {
    const name = p3.querySelector("h3, h2, .title")?.textContent?.trim() || "Not stated";
    const price = p3.querySelector(".price")?.textContent || p3.textContent || "";
    const introOffer = extractIntroOffer3(p3.textContent || "");
    const s2 = (p3.textContent || "").match(/NBN\s*(\d{1,4})/i)?.[1] ?? null;
    const parsedSpeed = s2 ? parseInt(s2) : null;
    const normalizedSpeed = normalizeSpeed(parsedSpeed);
    return {
      providerSlug: "dodo",
      planName: name,
      speedTier: normalizedSpeed,
      uploadSpeedMbps: extractUploadSpeed3(p3, normalizedSpeed),
      dataAllowance: extractDataAllowance3(p3),
      contractMonths: extractContractMonths3(p3),
      modemIncluded: extractModemIncluded3(p3),
      ...
      minTermDays: null,
      setupFeeCents: extractSetupFee3(p3),
      modemCostCents: null,
      conditionsText: (p3.textContent || "").trim() || null,
      promoDescription: introOffer.promoDescription,
      typicalEveningSpeedMbps: null,
...
var init_dodo = __esm({
  "../../packages/shared/dist/parsers/providers/dodo.js"() {
    ...
    init_validators();
    init_dom_utils();
    __name(extractUploadSpeed3, "extractUploadSpeed");
    __name(extractDataAllowance3, "extractDataAllowance");
    __name(extractContractMonths3, "extractContractMonths");
    __name(extractModemIncluded3, "extractModemIncluded");
    __name(extractSetupFee3, "extractSetupFee");
    __name(extractIntroOffer3, "extractIntroOffer");
    __name(canHandle3, "canHandle");
    __name(parse3, "parse");
  }
});

// ../../packages/shared/dist/parsers/providers/telstra.js
var telstra_exports = {};
__export(telstra_exports, {
  canHandle: () => canHandle4,
  parse: () => parse4
});
function canHandle4(url) {
  return url.includes("telstra.com") || url.includes("telstra.com.au");
}
function extractUploadSpeed4(planElement, downloadSpeed) {
  if (!downloadSpeed)
    return null;
  const planText = planElement.textContent || "";
  const uploadMatch = planText.match(/(\d+)\s*Mbps\s*(up|upload|↑)/i);
...
function extractDataAllowance4(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("unlimited"))
    return "Unlimited";
  const amountMatch = planText.match(/(\d+\.?\d*)\s*(tb|gb)/i);
...
function extractContractMonths4(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("no contract") || planText.includes("month-to-month"))
    return 0;
  if (planText.includes("24 month") || planText.includes("2 year"))
...
function extractModemIncluded4(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("modem included") || planText.includes("router included") || planText.includes("free modem"))
    return true;
  if (planText.includes("modem $") || planText.includes("bring your own") || planText.includes("your own modem"))
...
function extractSetupFee4(planElement) {
  const planText = planElement.textContent || "";
  if (planText.match(/free\s*(setup|connection|installation)/i))
    return 0;
  const feeMatch = planText.match(/(setup|connection|installation)[\s:]*\$(\d+(?:\.\d{2})?)/i);
...
function extractIntroOffer4(text) {
  const introMatch = text.match(/\$\s*([0-9]+(?:\.[0-9]{1,2})?)\s*(?:\/mo|\/month|per\s*month|pm)?\s*(?:for|first)\s*(\d{1,2})\s*(?:months|month|mo)/i);
  const thenMatch = text.match(/then\s*\$\s*([0-9]+(?:\.[0-9]{1,2})?)/i);
  const introPriceCents = introMatch ? parsePriceToCents(introMatch[1]) : null;
  const introDurationDays = introMatch ? parseInt(introMatch[2], 10) * 30 : null;
...
async function parse4(html, url) {
  const doc = parseHTML(html);
  ...
  const results = cards.map((el) => {
    ...
    const planText = el.textContent || "";
    const introOffer = extractIntroOffer4(planText);
    ...
    const uploadSpeed = extractUploadSpeed4(el, normalizedSpeed);
    const dataAllowance = extractDataAllowance4(el);
    const contractMonths = extractContractMonths4(el);
    const modemIncluded = extractModemIncluded4(el);
    const setupFee = extractSetupFee4(el);
    return {
...
var init_telstra = __esm({
  "../../packages/shared/dist/parsers/providers/telstra.js"() {
    "use strict";
    init_validators();
    init_dom_utils();
    __name(canHandle4, "canHandle");
    __name(extractUploadSpeed4, "extractUploadSpeed");
    __name(extractDataAllowance4, "extractDataAllowance");
    __name(extractContractMonths4, "extractContractMonths");
    __name(extractModemIncluded4, "extractModemIncluded");
    __name(extractSetupFee4, "extractSetupFee");
    __name(extractIntroOffer4, "extractIntroOffer");
    __name(parse4, "parse");
  }
});

// ../../packages/shared/dist/parsers/providers/optus.js
var optus_exports = {};
__export(optus_exports, {
  canHandle: () => canHandle5,
  parse: () => parse5
});
function extractUploadSpeed5(planElement, downloadSpeed) {
  if (!downloadSpeed)
    return null;
  const planText = planElement.textContent || "";
  const uploadMatch = planText.match(/(\d+)\s*Mbps\s*(up|upload|↑)/i);
...
function extractDataAllowance5(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("unlimited"))
    return "Unlimited";
  const amountMatch = planText.match(/(\d+\.?\d*)\s*(tb|gb)/i);
...
function extractContractMonths5(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("no contract") || planText.includes("month-to-month"))
    return 0;
  if (planText.includes("24 month") || planText.includes("2 year"))
...
function extractModemIncluded5(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("modem included") || planText.includes("router included") || planText.includes("free modem"))
    return true;
  if (planText.includes("modem $") || planText.includes("bring your own") || planText.includes("your own modem"))
...
function extractSetupFee5(planElement) {
  const planText = planElement.textContent || "";
  if (planText.match(/free\s*(setup|connection|installation)/i))
    return 0;
  const feeMatch = planText.match(/(setup|connection|installation)[\s:]*\$(\d+(?:\.\d{2})?)/i);
...
function extractIntroOffer5(text) {
  const introMatch = text.match(/\$\s*([0-9]+(?:\.[0-9]{1,2})?)\s*(?:\/mo|\/month|per\s*month|pm)?\s*(?:for|first)\s*(\d{1,2})\s*(?:months|month|mo)/i);
  const thenMatch = text.match(/then\s*\$\s*([0-9]+(?:\.[0-9]{1,2})?)/i);
  const introPriceCents = introMatch ? parsePriceToCents(introMatch[1]) : null;
  const introDurationDays = introMatch ? parseInt(introMatch[2], 10) * 30 : null;
...
async function parse5(html, url) {
  ...
  return cards.map((el) => {
    ...
    const priceText = (el.querySelector(".price")?.textContent || "").trim();
    const planText = el.textContent || "";
    const introOffer = extractIntroOffer5(planText);
    const speedMatch = (el.textContent || "").match(/(NBN\s*)?(\d{1,4})/i);
    ...
    return {
      ...
      planName: planName || "Not stated",
      speedTier: normalizedSpeed,
      uploadSpeedMbps: extractUploadSpeed5(el, normalizedSpeed),
      dataAllowance: extractDataAllowance5(el),
      contractMonths: extractContractMonths5(el),
      modemIncluded: extractModemIncluded5(el),
      introPriceCents: introOffer.introPriceCents ?? parsePriceToCents(priceText),
      introDurationDays: introOffer.introDurationDays,
      ongoingPriceCents: introOffer.thenPriceCents ?? parsePriceToCents(priceText),
      minTermDays: null,
      setupFeeCents: extractSetupFee5(el),
      modemCostCents: null,
      conditionsText: (el.querySelector(".terms")?.textContent || el.textContent || "").trim() || null,
      promoDescription: introOffer.promoDescription,
      typicalEveningSpeedMbps: null,
...
var init_optus = __esm({
  "../../packages/shared/dist/parsers/providers/optus.js"() {
    "use strict";
    init_validators();
    init_dom_utils();
    __name(extractUploadSpeed5, "extractUploadSpeed");
    __name(extractDataAllowance5, "extractDataAllowance");
    __name(extractContractMonths5, "extractContractMonths");
    __name(extractModemIncluded5, "extractModemIncluded");
    __name(extractSetupFee5, "extractSetupFee");
    __name(extractIntroOffer5, "extractIntroOffer");
    __name(canHandle5, "canHandle");
    __name(parse5, "parse");
  }
});

// ../../packages/shared/dist/parsers/providers/tpg.js
var tpg_exports = {};
__export(tpg_exports, {
  canHandle: () => canHandle6,
  parse: () => parse6
});
function extractUploadSpeed6(planElement, downloadSpeed) {
  if (!downloadSpeed)
    return null;
  const planText = planElement.textContent || "";
  const uploadMatch = planText.match(/(\d+)\s*Mbps\s*(up|upload|↑)/i);
...
function extractDataAllowance6(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("unlimited"))
    return "Unlimited";
  const amountMatch = planText.match(/(\d+\.?\d*)\s*(tb|gb)/i);
...
function extractContractMonths6(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("no contract") || planText.includes("month-to-month"))
    return 0;
  if (planText.includes("24 month") || planText.includes("2 year"))
...
function extractModemIncluded6(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("modem included") || planText.includes("router included") || planText.includes("free modem"))
    return true;
  if (planText.includes("modem $") || planText.includes("bring your own") || planText.includes("your own modem"))
...
function extractSetupFee6(planElement) {
  const planText = planElement.textContent || "";
  if (planText.match(/free\s*(setup|connection|installation)/i))
    return 0;
  const feeMatch = planText.match(/(setup|connection|installation)[\s:]*\$(\d+(?:\.\d{2})?)/i);
...
function extractIntroOffer6(text) {
  const introMatch = text.match(/\$\s*([0-9]+(?:\.[0-9]{1,2})?)\s*(?:\/mo|\/month|per\s*month|pm)?\s*(?:for|first)\s*(\d{1,2})\s*(?:months|month|mo)/i);
  const thenMatch = text.match(/then\s*\$\s*([0-9]+(?:\.[0-9]{1,2})?)/i);
  const introPriceCents = introMatch ? parsePriceToCents(introMatch[1]) : null;
  const introDurationDays = introMatch ? parseInt(introMatch[2], 10) * 30 : null;
...
function canHandle6(url) {
  return url.includes("tpg.com") || url.includes("tpg.com.au");
}
async function parse6(html, url) {
  const doc = parseHTML(html);
  ...
  return cards.map((el) => {
    ...
    const planText = el.textContent || "";
    const introOffer = extractIntroOffer6(planText);
    const speedMatch = (el.textContent || "").match(/NBN\s*(\d{1,4})/i) || (el.textContent || "").match(/(\d{1,4})\s*Mbps/i);
    const parsedSpeed = speedMatch ? parseInt(speedMatch[1]) : null;
    const normalizedSpeed = normalizeSpeed(parsedSpeed);
    return {
      ...
      planName: planName || "Not stated",
      speedTier: normalizedSpeed,
      uploadSpeedMbps: extractUploadSpeed6(el, normalizedSpeed),
      dataAllowance: extractDataAllowance6(el),
      contractMonths: extractContractMonths6(el),
      modemIncluded: extractModemIncluded6(el),
      introPriceCents: introOffer.introPriceCents ?? parsePriceToCents(priceText),
      introDurationDays: introOffer.introDurationDays,
      ongoingPriceCents: introOffer.thenPriceCents ?? parsePriceToCents(priceText),
      minTermDays: null,
      setupFeeCents: extractSetupFee6(el),
      modemCostCents: null,
      conditionsText: (el.querySelector(".terms")?.textContent || el.textContent || "").trim() || null,
      promoDescription: introOffer.promoDescription,
      typicalEveningSpeedMbps: null,
...
var init_tpg = __esm({
  "../../packages/shared/dist/parsers/providers/tpg.js"() {
    ...
    init_validators();
    init_dom_utils();
    __name(extractUploadSpeed6, "extractUploadSpeed");
    __name(extractDataAllowance6, "extractDataAllowance");
    __name(extractContractMonths6, "extractContractMonths");
    __name(extractModemIncluded6, "extractModemIncluded");
    __name(extractSetupFee6, "extractSetupFee");
    __name(extractIntroOffer6, "extractIntroOffer");
    __name(canHandle6, "canHandle");
    __name(parse6, "parse");
  }
});

// ../../packages/shared/dist/parsers/providers/vodafone.js
var vodafone_exports = {};
__export(vodafone_exports, {
  canHandle: () => canHandle7,
  parse: () => parse7
});
function extractUploadSpeed7(planElement, downloadSpeed) {
  if (!downloadSpeed)
    return null;
  const planText = planElement.textContent || "";
  const uploadMatch = planText.match(/(\d+)\s*Mbps\s*(up|upload|↑)/i);
...
function extractDataAllowance7(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("unlimited"))
    return "Unlimited";
  const amountMatch = planText.match(/(\d+\.?\d*)\s*(tb|gb)/i);
...
function extractContractMonths7(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("no contract") || planText.includes("month-to-month"))
    return 0;
  if (planText.includes("24 month") || planText.includes("2 year"))
...
function extractModemIncluded7(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("modem included") || planText.includes("router included") || planText.includes("free modem"))
    return true;
  if (planText.includes("modem $") || planText.includes("bring your own") || planText.includes("your own modem"))
...
function extractSetupFee7(planElement) {
  const planText = planElement.textContent || "";
  if (planText.match(/free\s*(setup|connection|installation)/i))
    return 0;
  const feeMatch = planText.match(/(setup|connection|installation)[\s:]*\$(\d+(?:\.\d{2})?)/i);
...
function extractIntroOffer7(text) {
  const introMatch = text.match(/\$\s*([0-9]+(?:\.[0-9]{1,2})?)\s*(?:\/mo|\/month|per\s*month|pm)?\s*(?:for|first)\s*(\d{1,2})\s*(?:months|month|mo)/i);
  const thenMatch = text.match(/then\s*\$\s*([0-9]+(?:\.[0-9]{1,2})?)/i);
  const introPriceCents = introMatch ? parsePriceToCents(introMatch[1]) : null;
  const introDurationDays = introMatch ? parseInt(introMatch[2], 10) * 30 : null;
...
function canHandle7(url) {
  return url.includes("vodafone.com") || url.includes("vodafone.com.au");
}
async function parse7(html, url) {
  const doc = parseHTML(html);
  const cards = Array.from(doc.querySelectorAll(".offer, .product, .plan-card"));
  return cards.map((el) => {
    const planName = (el.querySelector("h3")?.textContent || el.querySelector(".title")?.textContent || "").trim();
    const priceText = (el.querySelector(".price")?.textContent || "").trim();
    const planText = el.textContent || "";
    const introOffer = extractIntroOffer7(planText);
    const speedMatch = (el.textContent || "").match(/NBN\s*(\d{1,4})/i) || (el.textContent || "").match(/(\d{1,4})\s*Mbps/i);
    const parsedSpeed = speedMatch ? parseInt(speedMatch[1]) : null;
    const normalizedSpeed = normalizeSpeed(parsedSpeed);
    return {
      ...
      planName: planName || "Not stated",
      speedTier: normalizedSpeed,
      uploadSpeedMbps: extractUploadSpeed7(el, normalizedSpeed),
      dataAllowance: extractDataAllowance7(el),
      contractMonths: extractContractMonths7(el),
      modemIncluded: extractModemIncluded7(el),
      introPriceCents: introOffer.introPriceCents ?? parsePriceToCents(priceText),
      introDurationDays: introOffer.introDurationDays,
      ongoingPriceCents: introOffer.thenPriceCents ?? parsePriceToCents(priceText),
      minTermDays: null,
      setupFeeCents: extractSetupFee7(el),
      modemCostCents: null,
      conditionsText: (el.querySelector(".terms")?.textContent || el.textContent || "").trim() || null,
      promoDescription: introOffer.promoDescription,
      typicalEveningSpeedMbps: null,
...
var init_vodafone = __esm({
  "../../packages/shared/dist/parsers/providers/vodafone.js"() {
    "use strict";
    init_validators();
    init_dom_utils();
    __name(extractUploadSpeed7, "extractUploadSpeed");
    __name(extractDataAllowance7, "extractDataAllowance");
    __name(extractContractMonths7, "extractContractMonths");
    __name(extractModemIncluded7, "extractModemIncluded");
    __name(extractSetupFee7, "extractSetupFee");
    __name(extractIntroOffer7, "extractIntroOffer");
    __name(canHandle7, "canHandle");
    __name(parse7, "parse");
  }
});

// ../../packages/shared/dist/parsers/providers/superloop.js
var superloop_exports = {};
__export(superloop_exports, {
  canHandle: () => canHandle8,
  parse: () => parse8
});
function extractUploadSpeed8(planElement, downloadSpeed) {
  if (!downloadSpeed)
    return null;
  const planText = planElement.textContent || "";
  const uploadMatch = planText.match(/(\d+)\s*Mbps\s*(up|upload|↑)/i);
...
function extractDataAllowance8(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("unlimited"))
    return "Unlimited";
  const amountMatch = planText.match(/(\d+\.?\d*)\s*(tb|gb)/i);
...
function extractContractMonths8(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("no contract") || planText.includes("month-to-month"))
    return 0;
  if (planText.includes("24 month") || planText.includes("2 year"))
...
function extractModemIncluded8(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("modem included") || planText.includes("router included") || planText.includes("free modem"))
    return true;
  if (planText.includes("modem $") || planText.includes("bring your own") || planText.includes("your own modem"))
...
function extractSetupFee8(planElement) {
  const planText = planElement.textContent || "";
  if (planText.match(/free\s*(setup|connection|installation)/i))
    return 0;
  const feeMatch = planText.match(/(setup|connection|installation)[\s:]*\$(\d+(?:\.\d{2})?)/i);
...
function extractIntroOffer8(text) {
  const introMatch = text.match(/\$\s*([0-9]+(?:\.[0-9]{1,2})?)\s*(?:\/mo|\/month|per\s*month|pm)?\s*(?:for|first)\s*(\d{1,2})\s*(?:months|month|mo)/i);
  const thenMatch = text.match(/then\s*\$\s*([0-9]+(?:\.[0-9]{1,2})?)/i);
  const introPriceCents = introMatch ? parsePriceToCents(introMatch[1]) : null;
  const introDurationDays = introMatch ? parseInt(introMatch[2], 10) * 30 : null;
...
async function parse8(html, url) {
  ...
  const planPatterns = [
    ...
    { name: "Lightspeed", speed: 1e3, upload: 100, ongoing: 109, intro: 85, introDays: 180 },
    { name: "Hyperspeed", speed: 2e3, upload: 200, ongoing: 165, intro: 145, introDays: 180 }
  ];
  for (const pattern of planPatterns) {
    ...
    if (planNameInContent || speedInContent && priceInContent) {
      plans.push({
        ...
        planName: `Superloop NBN ${pattern.speed}`,
        // Match existing DB format
  ...
  if (plans.length === 0) {
    ...
    plans.push(...cards.map((el) => {
      const planName = (el.querySelector("h3")?.textContent || el.querySelector(".title")?.textContent || "").trim();
      const priceText = (el.querySelector(".price")?.textContent || "").trim();
      const planText = el.textContent || "";
      const introOffer = extractIntroOffer8(planText);
      const speedMatch = (el.textContent || "").match(/NBN\s*(\d{1,4})/i) || (el.textContent || "").match(/(\d{1,4})\s*Mbps/i);
      const parsedSpeed = speedMatch ? parseInt(speedMatch[1]) : null;
      const normalizedSpeed = normalizeSpeed(parsedSpeed);
      return {
        ...
        planName: planName || "Not stated",
        speedTier: normalizedSpeed,
        uploadSpeedMbps: extractUploadSpeed8(el, normalizedSpeed),
        dataAllowance: extractDataAllowance8(el),
        contractMonths: extractContractMonths8(el),
        modemIncluded: extractModemIncluded8(el),
        introPriceCents: introOffer.introPriceCents ?? parsePriceToCents(priceText),
        introDurationDays: introOffer.introDurationDays,
        ongoingPriceCents: introOffer.thenPriceCents ?? parsePriceToCents(priceText),
        minTermDays: null,
        setupFeeCents: extractSetupFee8(el),
        modemCostCents: null,
        conditionsText: (el.querySelector(".terms")?.textContent || el.textContent || "").trim() || null,
        promoDescription: introOffer.promoDescription,
        typicalEveningSpeedMbps: null,
...
var init_superloop = __esm({
  "../../packages/shared/dist/parsers/providers/superloop.js"() {
    "use strict";
    init_validators();
    init_dom_utils();
    __name(extractUploadSpeed8, "extractUploadSpeed");
    __name(extractDataAllowance8, "extractDataAllowance");
    __name(extractContractMonths8, "extractContractMonths");
    __name(extractModemIncluded8, "extractModemIncluded");
    __name(extractSetupFee8, "extractSetupFee");
    __name(extractIntroOffer8, "extractIntroOffer");
    __name(canHandle8, "canHandle");
    __name(parse8, "parse");
  }
});

// ../../packages/shared/dist/parsers/providers/kogan.js
var kogan_exports = {};
__export(kogan_exports, {
  canHandle: () => canHandle9,
  parse: () => parse9
});
function extractUploadSpeed9(planElement, downloadSpeed) {
  if (!downloadSpeed)
    return null;
  const planText = planElement.textContent || "";
  const uploadMatch = planText.match(/(\d+)\s*Mbps\s*(up|upload|↑)/i);
...
function extractDataAllowance9(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("unlimited"))
    return "Unlimited";
  const amountMatch = planText.match(/(\d+\.?\d*)\s*(tb|gb)/i);
...
function extractContractMonths9(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("no contract") || planText.includes("month-to-month"))
    return 0;
  if (planText.includes("24 month") || planText.includes("2 year"))
...
function extractModemIncluded9(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("modem included") || planText.includes("router included") || planText.includes("free modem"))
    return true;
  if (planText.includes("modem $") || planText.includes("bring your own") || planText.includes("your own modem"))
...
function extractSetupFee9(planElement) {
  const planText = planElement.textContent || "";
  if (planText.match(/free\s*(setup|connection|installation)/i))
    return 0;
  const feeMatch = planText.match(/(setup|connection|installation)[\s:]*\$(\d+(?:\.\d{2})?)/i);
...
function extractIntroOffer9(text) {
  const introMatch = text.match(/\$\s*([0-9]+(?:\.[0-9]{1,2})?)\s*(?:\/mo|\/month|per\s*month|pm)?\s*(?:for|first)\s*(\d{1,2})\s*(?:months|month|mo)/i);
  const thenMatch = text.match(/then\s*\$\s*([0-9]+(?:\.[0-9]{1,2})?)/i);
  const introPriceCents = introMatch ? parsePriceToCents(introMatch[1]) : null;
  const introDurationDays = introMatch ? parseInt(introMatch[2], 10) * 30 : null;
...
function canHandle9(url) {
  return url.includes("kogan.com") || url.includes("kogan.com.au");
}
async function parse9(html, url) {
  const doc = parseHTML(html);
  const cards = Array.from(doc.querySelectorAll(".plan, .product, .offer"));
  return cards.map((el) => {
    const planName = (el.querySelector("h3")?.textContent || el.querySelector(".title")?.textContent || "").trim();
    const priceText = (el.querySelector(".price")?.textContent || "").trim();
    const introOffer = extractIntroOffer9(el.textContent || "");
    const speedMatch = (el.textContent || "").match(/NBN\s*(\d{1,4})/i) || (el.textContent || "").match(/(\d{1,4})\s*Mbps/i);
    const parsedSpeed = speedMatch ? parseInt(speedMatch[1]) : null;
    const normalizedSpeed = normalizeSpeed(parsedSpeed);
    return {
      ...
      planName: planName || "Not stated",
      speedTier: normalizedSpeed,
      uploadSpeedMbps: extractUploadSpeed9(el, normalizedSpeed),
      dataAllowance: extractDataAllowance9(el),
      contractMonths: extractContractMonths9(el),
      modemIncluded: extractModemIncluded9(el),
      introPriceCents: introOffer.introPriceCents ?? parsePriceToCents(priceText),
      introDurationDays: introOffer.introDurationDays,
      ongoingPriceCents: introOffer.thenPriceCents ?? parsePriceToCents(priceText),
      minTermDays: null,
      setupFeeCents: extractSetupFee9(el),
      modemCostCents: null,
      conditionsText: (el.querySelector(".terms")?.textContent || el.textContent || "").trim() || null,
      promoDescription: introOffer.promoDescription,
      typicalEveningSpeedMbps: null,
...
var init_kogan = __esm({
  "../../packages/shared/dist/parsers/providers/kogan.js"() {
    "use strict";
    init_validators();
    init_dom_utils();
    __name(extractUploadSpeed9, "extractUploadSpeed");
    __name(extractDataAllowance9, "extractDataAllowance");
    __name(extractContractMonths9, "extractContractMonths");
    __name(extractModemIncluded9, "extractModemIncluded");
    __name(extractSetupFee9, "extractSetupFee");
    __name(extractIntroOffer9, "extractIntroOffer");
    __name(canHandle9, "canHandle");
    __name(parse9, "parse");
  }
});

// ../../packages/shared/dist/parsers/providers/foxtel.js
var foxtel_exports = {};
__export(foxtel_exports, {
  canHandle: () => canHandle10,
  parse: () => parse10
});
function extractUploadSpeed10(planElement, downloadSpeed) {
  if (!downloadSpeed)
    return null;
  const planText = planElement.textContent || "";
  const uploadMatch = planText.match(/(\d+)\s*Mbps\s*(up|upload|↑)/i);
...
function extractDataAllowance10(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("unlimited"))
    return "Unlimited";
  const amountMatch = planText.match(/(\d+\.?\d*)\s*(tb|gb)/i);
...
function extractContractMonths10(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("no contract") || planText.includes("month-to-month"))
    return 0;
  if (planText.includes("24 month") || planText.includes("2 year"))
...
function extractModemIncluded10(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("modem included") || planText.includes("router included") || planText.includes("free modem"))
    return true;
  if (planText.includes("modem $") || planText.includes("bring your own") || planText.includes("your own modem"))
...
function extractSetupFee10(planElement) {
  const planText = planElement.textContent || "";
  if (planText.match(/free\s*(setup|connection|installation)/i))
    return 0;
  const feeMatch = planText.match(/(setup|connection|installation)[\s:]*\$(\d+(?:\.\d{2})?)/i);
...
async function parse10(html, url) {
  ...
  return cards.map((el) => {
    ...
    return {
      ...
      planName: planName || "Not stated",
      speedTier: normalizedSpeed,
      uploadSpeedMbps: extractUploadSpeed10(el, normalizedSpeed),
      dataAllowance: extractDataAllowance10(el),
      contractMonths: extractContractMonths10(el),
      modemIncluded: extractModemIncluded10(el),
      introPriceCents: parsePriceToCents(priceText),
      introDurationDays: null,
      ongoingPriceCents: parsePriceToCents(priceText),
      minTermDays: null,
      setupFeeCents: extractSetupFee10(el),
      modemCostCents: null,
      conditionsText: (el.querySelector(".terms")?.textContent || el.textContent || "").trim() || null,
      typicalEveningSpeedMbps: null,
      sourceUrl: url
...
var init_foxtel = __esm({
  "../../packages/shared/dist/parsers/providers/foxtel.js"() {
    "use strict";
    init_validators();
    init_dom_utils();
    __name(extractUploadSpeed10, "extractUploadSpeed");
    __name(extractDataAllowance10, "extractDataAllowance");
    __name(extractContractMonths10, "extractContractMonths");
    __name(extractModemIncluded10, "extractModemIncluded");
    __name(extractSetupFee10, "extractSetupFee");
    __name(canHandle10, "canHandle");
    __name(parse10, "parse");
  }
});

// ../../packages/shared/dist/parsers/providers/exetel.js
var exetel_exports = {};
__export(exetel_exports, {
  canHandle: () => canHandle11,
  parse: () => parse11
});
function extractUploadSpeed11(planElement, downloadSpeed) {
  if (!planElement || !downloadSpeed)
    return null;
  const planText = planElement.textContent || "";
  const uploadMatch = planText.match(/(\d+)\s*Mbps\s*(up|upload|↑)/i);
  if (uploadMatch) {
...
function extractDataAllowance11(planElement) {
  const planText = (planElement?.textContent || "").toLowerCase();
  if (planText.includes("unlimited"))
    return "Unlimited";
  const amountMatch = planText.match(/(\d+\.?\d*)\s*(tb|gb)/i);
...
function extractContractMonths11(planElement) {
  const planText = (planElement?.textContent || "").toLowerCase();
  if (planText.includes("no contract") || planText.includes("month-to-month"))
    return 0;
  if (planText.includes("24 month") || planText.includes("2 year"))
...
function extractModemIncluded11(planElement) {
  const planText = (planElement?.textContent || "").toLowerCase();
  if (planText.includes("modem included") || planText.includes("router included") || planText.includes("free modem"))
    return true;
  if (planText.includes("modem $") || planText.includes("bring your own") || planText.includes("your own modem"))
...
function extractSetupFee11(planElement) {
  const planText = planElement?.textContent || "";
  if (planText.match(/free\s*(setup|connection|installation)/i))
    return 0;
  const feeMatch = planText.match(/(setup|connection|installation)[\s:]*\$(\d+(?:\.\d{2})?)/i);
...
function extractIntroOffer10(text) {
  const introMatch = text.match(/\$\s*([0-9]+(?:\.[0-9]{1,2})?)\s*(?:\/mo|\/month|per\s*month|pm)?\s*(?:for|first)\s*(\d{1,2})\s*(?:months|month|mo)/i);
  const thenMatch = text.match(/then\s*\$\s*([0-9]+(?:\.[0-9]{1,2})?)/i);
  const introPriceCents = introMatch ? parsePriceToCents(introMatch[1]) : null;
  const introDurationDays = introMatch ? parseInt(introMatch[2], 10) * 30 : null;
...
function canHandle11(url) {
  return /exetel\.com\.au/i.test(url);
}
async function parse11(html, url) {
  const doc = parseHTML(html);
  const results = [];
  ...
  for (const element of Array.from(planElements)) {
    try {
      ...
      if (!planName) {
        const nameMatch = text.match(/Exetel\s+(?:NBN|FW)\s+\d+/i) || text.match(/The\s+One\s+Plan/i) || text.match(/NBN\s+\d+/i);
        if (nameMatch) {
      ...
      const introOffer = extractIntroOffer10(element.textContent || "");
      ...
      results.push({
        ...
        planName,
        speedTier,
        uploadSpeedMbps: extractUploadSpeed11(element, speedTier),
        dataAllowance: extractDataAllowance11(element),
        contractMonths: extractContractMonths11(element),
        modemIncluded: extractModemIncluded11(element),
        introPriceCents: introOffer.introPriceCents ?? priceCents,
        introDurationDays: introOffer.introDurationDays,
        ongoingPriceCents: introOffer.thenPriceCents ?? priceCents,
        minTermDays: null,
        setupFeeCents: extractSetupFee11(element),
        modemCostCents: null,
  ...
  if (results.length === 0) {
    ...
    for (let i2 = 0; i2 < lines.length; i2++) {
      ...
      let priceText = "";
      for (let j = i2; j < Math.min(i2 + 5, lines.length); j++) {
...
var init_exetel = __esm({
  "../../packages/shared/dist/parsers/providers/exetel.js"() {
    "use strict";
    init_validators();
    init_dom_utils();
    __name(extractUploadSpeed11, "extractUploadSpeed");
    __name(extractDataAllowance11, "extractDataAllowance");
    __name(extractContractMonths11, "extractContractMonths");
    __name(extractModemIncluded11, "extractModemIncluded");
    __name(extractSetupFee11, "extractSetupFee");
    __name(extractIntroOffer10, "extractIntroOffer");
    __name(canHandle11, "canHandle");
    __name(parse11, "parse");
  }
});

// ../../packages/shared/dist/parsers/providers/arctel.js
var arctel_exports = {};
__export(arctel_exports, {
  canHandle: () => canHandle12,
  parse: () => parse12
});
function extractUploadSpeed12(planElement, downloadSpeed) {
  if (!downloadSpeed)
    return null;
  const planText = planElement?.textContent || "";
  const uploadMatch = planText.match(/(\d+)\s*Mbps\s*(up|upload|↑)/i);
...
function extractDataAllowance12(planElement) {
  const planText = (planElement?.textContent || "").toLowerCase();
  if (planText.includes("unlimited"))
    return "Unlimited";
  const amountMatch = planText.match(/(\d+\.?\d*)\s*(tb|gb)/i);
...
function extractContractMonths12(planElement) {
  const planText = (planElement?.textContent || "").toLowerCase();
  if (planText.includes("no contract") || planText.includes("month-to-month"))
    return 0;
  if (planText.includes("24 month") || planText.includes("2 year"))
...
function extractModemIncluded12(planElement) {
  const planText = (planElement?.textContent || "").toLowerCase();
  if (planText.includes("modem included") || planText.includes("router included") || planText.includes("free modem"))
    return true;
  if (planText.includes("modem $") || planText.includes("bring your own") || planText.includes("your own modem"))
...
function extractSetupFee12(planElement) {
  const planText = planElement?.textContent || "";
  if (planText.match(/free\s*(setup|connection|installation)/i))
    return 0;
  const feeMatch = planText.match(/(setup|connection|installation)[\s:]*\$(\d+(?:\.\d{2})?)/i);
...
function canHandle12(url) {
  return url.includes("arctel.com.au");
}
async function parse12(html, url) {
  const doc = parseHTML(html);
  const results = [];
  const planElements = Array.from(doc.querySelectorAll(".plan-card, .plan-box, .pricing-card, .nbn-plan, [class*='plan'], [class*='pricing']"));
  for (const element of planElements) {
    const text = element.textContent || "";
    if (text.length < 20)
      continue;
    const speedMatch = text.match(/NBN\s*(\d{3,4})/i) || text.match(/(\d{3,4})\s*(?:Mbps|mbps)/i) || text.match(/(\d)\s*Gbps/i);
    if (!speedMatch)
      continue;
    let speedValue = parseInt(speedMatch[1]);
    if (text.match(/Gbps/i)) {
      speedValue = speedValue * 1e3;
    }
    const speedTier = normalizeSpeed(speedValue);
    if (!speedTier)
      continue;
    ...
    const dealMatch = text.match(/\$\s*(\d+(?:\.\d{2})?)\s*(?:\/)?(?:month|mo|p\.?m\.?)?(?:\s+for\s+)?(\d+)\s*(?:month|mo)/i);
    if (dealMatch) {
    ...
    if (!introPriceCents && !ongoingPriceCents)
      continue;
    ...
    results.push({
      ...
      speedTier,
      uploadSpeedMbps: extractUploadSpeed12(element, speedTier),
      dataAllowance: extractDataAllowance12(element),
      contractMonths: extractContractMonths12(element),
      modemIncluded: extractModemIncluded12(element),
      introPriceCents: introPriceCents || ongoingPriceCents || null,
      introDurationDays,
      ongoingPriceCents: ongoingPriceCents || introPriceCents || null,
      minTermDays: null,
      setupFeeCents: extractSetupFee12(element),
  ...
  if (results.length === 0) {
    const lines = html.replace(/<script[\s\S]*?<\/script>/gi, "").replace(/<style[\s\S]*?<\/style>/gi, "").split(/\n|<br>/i);
    for (let i2 = 0; i2 < lines.length; i2++) {
      const line = lines[i2];
      const nextLine = i2 + 1 < lines.length ? lines[i2 + 1] : "";
      const chunk = line + " " + nextLine;
      const speedMatch = chunk.match(/NBN\s*(\d{3,4})/i) || chunk.match(/(\d{3,4})\s*Mbps/i);
      if (!speedMatch)
        continue;
...
var init_arctel = __esm({
  "../../packages/shared/dist/parsers/providers/arctel.js"() {
    "use strict";
    init_validators();
    init_dom_utils();
    __name(extractUploadSpeed12, "extractUploadSpeed");
    __name(extractDataAllowance12, "extractDataAllowance");
    __name(extractContractMonths12, "extractContractMonths");
    __name(extractModemIncluded12, "extractModemIncluded");
    __name(extractSetupFee12, "extractSetupFee");
    __name(canHandle12, "canHandle");
    __name(parse12, "parse");
  }
});

// ../../packages/shared/dist/parsers/providers/launtel.js
var launtel_exports = {};
__export(launtel_exports, {
  canHandle: () => canHandle13,
  parse: () => parse13
});
function extractUploadSpeed13(planElement, downloadSpeed) {
  if (!planElement || !downloadSpeed)
    return null;
  const planText = planElement.textContent || "";
  const uploadMatch = planText.match(/(\d+)\s*Mbps\s*(up|upload|↑)/i);
...
function extractDataAllowance13(planElement) {
  const planText = (planElement?.textContent || "").toLowerCase();
  if (planText.includes("unlimited"))
    return "Unlimited";
  const amountMatch = planText.match(/(\d+\.?\d*)\s*(tb|gb)/i);
...
function extractContractMonths13(planElement) {
  const planText = (planElement?.textContent || "").toLowerCase();
  if (planText.includes("no contract") || planText.includes("month-to-month"))
    return 0;
  if (planText.includes("24 month") || planText.includes("2 year"))
...
function extractModemIncluded13(planElement) {
  const planText = (planElement?.textContent || "").toLowerCase();
  if (planText.includes("modem included") || planText.includes("router included") || planText.includes("free modem"))
    return true;
  if (planText.includes("modem $") || planText.includes("bring your own") || planText.includes("your own modem"))
...
function extractSetupFee13(planElement) {
  const planText = planElement?.textContent || "";
  if (planText.match(/free\s*(setup|connection|installation)/i))
    return 0;
  const feeMatch = planText.match(/(setup|connection|installation)[\s:]*\$(\d+(?:\.\d{2})?)/i);
...
function canHandle13(url) {
  return url.includes("launtel.net.au") || url.includes("launtel.com");
}
async function parse13(html, url) {
  const doc = parseHTML(html);
  const results = [];
  let rows = Array.from(doc.querySelectorAll(".plan, .plan-card, [class*='plan-']"));
  if (rows.length === 0) {
    rows = Array.from(doc.querySelectorAll("h3, h4")).filter((h2) => /NBN|plan|speed/i.test(h2.textContent || "")).map((h2) => h2.parentElement).filter((el) => !!el);
  }
  for (const el of rows) {
    const speedText = (el.textContent || "").match(/NBN\s*(\d{1,4})/i) || (el.textContent || "").match(/(\d{1,4})\s*Mbps/i);
    if (!speedText)
      continue;
    ...
    results.push({
      ...
      speedTier,
      uploadSpeedMbps: extractUploadSpeed13(el, speedTier),
      dataAllowance: extractDataAllowance13(el),
      contractMonths: extractContractMonths13(el),
      modemIncluded: extractModemIncluded13(el),
      introPriceCents: parsePriceToCents(priceText),
      introDurationDays: null,
      ongoingPriceCents: parsePriceToCents(priceText),
      minTermDays: null,
      setupFeeCents: extractSetupFee13(el),
      modemCostCents: null,
      conditionsText: (el.textContent || "").trim() || null,
      typicalEveningSpeedMbps: null,
      sourceUrl: url
...
var init_launtel = __esm({
  "../../packages/shared/dist/parsers/providers/launtel.js"() {
    "use strict";
    init_validators();
    init_dom_utils();
    __name(extractUploadSpeed13, "extractUploadSpeed");
    __name(extractDataAllowance13, "extractDataAllowance");
    __name(extractContractMonths13, "extractContractMonths");
    __name(extractModemIncluded13, "extractModemIncluded");
    __name(extractSetupFee13, "extractSetupFee");
    __name(canHandle13, "canHandle");
    __name(parse13, "parse");
  }
});

// ../../packages/shared/dist/parsers/providers/leaptel.js
var leaptel_exports = {};
__export(leaptel_exports, {
  canHandle: () => canHandle14,
  parse: () => parse14
});
function extractUploadSpeed14(planElement, downloadSpeed) {
  if (!planElement || !downloadSpeed)
    return null;
  const planText = planElement.textContent || "";
  const uploadMatch = planText.match(/(\d+)\s*Mbps\s*(up|upload|↑)/i);
...
function extractDataAllowance14(planElement) {
  const planText = (planElement?.textContent || "").toLowerCase();
  if (planText.includes("unlimited"))
    return "Unlimited";
  const amountMatch = planText.match(/(\d+\.?\d*)\s*(tb|gb)/i);
...
function extractContractMonths14(planElement) {
  const planText = (planElement?.textContent || "").toLowerCase();
  if (planText.includes("no contract") || planText.includes("month-to-month"))
    return 0;
  if (planText.includes("24 month") || planText.includes("2 year"))
...
function extractModemIncluded14(planElement) {
  const planText = (planElement?.textContent || "").toLowerCase();
  if (planText.includes("modem included") || planText.includes("router included") || planText.includes("free modem"))
    return true;
  if (planText.includes("modem $") || planText.includes("bring your own") || planText.includes("your own modem"))
...
function extractSetupFee14(planElement) {
  const planText = planElement?.textContent || "";
  if (planText.match(/free\s*(setup|connection|installation)/i))
    return 0;
  const feeMatch = planText.match(/(setup|connection|installation)[\s:]*\$(\d+(?:\.\d{2})?)/i);
...
function canHandle14(url) {
  return url.includes("leaptel.com.au") || url.includes("leaptel.net.au");
}
async function parse14(html, url) {
  const doc = parseHTML(html);
  const results = [];
  let rows = Array.from(doc.querySelectorAll(".plan, .plan-card, [class*='plan-'], .pricing-card"));
  if (rows.length === 0) {
    rows = Array.from(doc.querySelectorAll("h3, h4")).filter((h2) => /NBN|plan|speed|pricing/i.test(h2.textContent || "")).map((h2) => h2.parentElement).filter((el) => !!el);
  }
  for (const el of rows) {
    const speedText = (el.textContent || "").match(/NBN\s*(\d{1,4})/i) || (el.textContent || "").match(/(\d{1,4})\s*Mbps/i);
    if (!speedText)
    ...
    results.push({
      ...
      planName: (el.querySelector("h4, h5, .name, [class*='title']")?.textContent || "").trim() || `NBN ${speedTier}`,
      speedTier,
      uploadSpeedMbps: extractUploadSpeed14(el, speedTier),
      dataAllowance: extractDataAllowance14(el),
      contractMonths: extractContractMonths14(el),
      modemIncluded: extractModemIncluded14(el),
      introPriceCents: parsePriceToCents(priceText),
      introDurationDays: null,
      ongoingPriceCents: parsePriceToCents(priceText),
      minTermDays: null,
      setupFeeCents: extractSetupFee14(el),
      modemCostCents: null,
      conditionsText: (el.textContent || "").trim() || null,
      typicalEveningSpeedMbps: null,
      sourceUrl: url
...
var init_leaptel = __esm({
  "../../packages/shared/dist/parsers/providers/leaptel.js"() {
    "use strict";
    init_validators();
    init_dom_utils();
    __name(extractUploadSpeed14, "extractUploadSpeed");
    __name(extractDataAllowance14, "extractDataAllowance");
    __name(extractContractMonths14, "extractContractMonths");
    __name(extractModemIncluded14, "extractModemIncluded");
    __name(extractSetupFee14, "extractSetupFee");
    __name(canHandle14, "canHandle");
    __name(parse14, "parse");
  }
});

// ../../packages/shared/dist/parsers/providers/myrepublic.js
var myrepublic_exports = {};
__export(myrepublic_exports, {
  canHandle: () => canHandle15,
  parse: () => parse15
});
function extractUploadSpeed15(planElement, downloadSpeed) {
  if (!planElement || !downloadSpeed)
    return null;
  const planText = planElement.textContent || "";
  const uploadMatch = planText.match(/(\d+)\s*Mbps\s*(up|upload|↑)/i);
...
function extractDataAllowance15(planElement) {
  const planText = (planElement?.textContent || "").toLowerCase();
  if (planText.includes("unlimited"))
    return "Unlimited";
  const amountMatch = planText.match(/(\d+\.?\d*)\s*(tb|gb)/i);
...
function extractContractMonths15(planElement) {
  const planText = (planElement?.textContent || "").toLowerCase();
  if (planText.includes("no contract") || planText.includes("month-to-month"))
    return 0;
  if (planText.includes("24 month") || planText.includes("2 year"))
...
function extractModemIncluded15(planElement) {
  const planText = (planElement?.textContent || "").toLowerCase();
  if (planText.includes("modem included") || planText.includes("router included") || planText.includes("free modem"))
    return true;
  if (planText.includes("modem $") || planText.includes("bring your own") || planText.includes("your own modem"))
...
function extractSetupFee15(planElement) {
  const planText = planElement?.textContent || "";
  if (planText.match(/free\s*(setup|connection|installation)/i))
    return 0;
  const feeMatch = planText.match(/(setup|connection|installation)[\s:]*\$(\d+(?:\.\d{2})?)/i);
...
function canHandle15(url) {
  return url.includes("myrepublic.com.au");
}
async function parse15(html, url) {
  const doc = parseHTML(html);
  const results = [];
  let rows = Array.from(doc.querySelectorAll(".plan, .plan-item, .pricing-tier, [data-plan], [class*='plan-box']"));
  if (rows.length === 0) {
    rows = Array.from(doc.querySelectorAll("h3, h4")).filter((h2) => /NBN|plan|speed|pricing/i.test(h2.textContent || "")).map((h2) => h2.closest("div")).filter((el) => !!el && (el.textContent || "").length > 50);
  }
  for (const el of rows) {
    const speedText = (el.textContent || "").match(/NBN\s*(\d{1,4})/i) || (el.textContent || "").match(/(\d{1,4})\s*Mbps/i);
    if (!speedText)
    ...
    if (!speedTier)
      continue;
    const priceText = el.querySelector(".price, .plan-price, [class*='price']")?.textContent || el.textContent || "";
    results.push({
      providerSlug: "myrepublic",
      planName: (el.querySelector("h4, h5, .plan-name, [class*='name']")?.textContent || "").trim() || `NBN ${speedTier}`,
      speedTier,
      uploadSpeedMbps: extractUploadSpeed15(el, speedTier),
      dataAllowance: extractDataAllowance15(el),
      contractMonths: extractContractMonths15(el),
      modemIncluded: extractModemIncluded15(el),
      introPriceCents: parsePriceToCents(priceText),
      introDurationDays: null,
      ongoingPriceCents: parsePriceToCents(priceText),
      minTermDays: null,
      setupFeeCents: extractSetupFee15(el),
      modemCostCents: null,
      conditionsText: (el.textContent || "").trim() || null,
      typicalEveningSpeedMbps: null,
      sourceUrl: url
...
var init_myrepublic = __esm({
  "../../packages/shared/dist/parsers/providers/myrepublic.js"() {
    "use strict";
    init_validators();
    init_dom_utils();
    __name(extractUploadSpeed15, "extractUploadSpeed");
    __name(extractDataAllowance15, "extractDataAllowance");
    __name(extractContractMonths15, "extractContractMonths");
    __name(extractModemIncluded15, "extractModemIncluded");
    __name(extractSetupFee15, "extractSetupFee");
    __name(canHandle15, "canHandle");
    __name(parse15, "parse");
  }
});

// ../../packages/shared/dist/parsers/providers/iinet.js
var iinet_exports = {};
__export(iinet_exports, {
  canHandle: () => canHandle16,
  parse: () => parse16
});
function extractUploadSpeed16(planElement, downloadSpeed) {
  if (!downloadSpeed)
    return null;
  const planText = planElement.textContent || "";
  const uploadMatch = planText.match(/(\d+)\s*Mbps\s*(up|upload|↑)/i);
...
function extractDataAllowance16(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("unlimited"))
    return "Unlimited";
  const amountMatch = planText.match(/(\d+\.?\d*)\s*(tb|gb)/i);
...
function extractContractMonths16(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("no contract") || planText.includes("month-to-month"))
    return 0;
  if (planText.includes("24 month") || planText.includes("2 year"))
...
function extractModemIncluded16(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("modem included") || planText.includes("router included") || planText.includes("free modem"))
    return true;
  if (planText.includes("modem $") || planText.includes("bring your own") || planText.includes("your own modem"))
...
function extractSetupFee16(planElement) {
  const planText = planElement.textContent || "";
  if (planText.match(/free\s*(setup|connection|installation)/i))
    return 0;
  const feeMatch = planText.match(/(setup|connection|installation)[\s:]*\$(\d+(?:\.\d{2})?)/i);
...
function extractIntroOffer11(text) {
  const introMatch = text.match(/\$\s*([0-9]+(?:\.[0-9]{1,2})?)\s*(?:\/mo|\/month|per\s*month|pm)?\s*(?:for|first)\s*(\d{1,2})\s*(?:months|month|mo)/i);
  const thenMatch = text.match(/then\s*\$\s*([0-9]+(?:\.[0-9]{1,2})?)/i);
  const introPriceCents = introMatch ? parsePriceToCents(introMatch[1]) : null;
  const introDurationDays = introMatch ? parseInt(introMatch[2], 10) * 30 : null;
...
function canHandle16(url) {
  return url.includes("iinet.net.au") || url.includes("iinet.com.au");
}
async function parse16(html, url) {
  const doc = parseHTML(html);
  const results = [];
  let rows = Array.from(doc.querySelectorAll(".plan, .plan-box, .product-item, [class*='plan-']"));
  if (rows.length === 0) {
    rows = Array.from(doc.querySelectorAll("section, .pricing-section, [class*='plan-section']")).filter((el) => /NBN|speed|plan/i.test(el.textContent || ""));
  }
  for (const el of rows) {
    const speedText = (el.textContent || "").match(/NBN\s*(\d{1,4})/i) || (el.textContent || "").match(/(\d{1,4})\s*Mbps/i);
    if (!speedText)
    ...
    if (!speedTier)
      continue;
    const priceText = el.querySelector(".price, .plan-price, [class*='price']")?.textContent || el.textContent || "";
    const introOffer = extractIntroOffer11(el.textContent || "");
    results.push({
      providerSlug: "iinet",
      planName: (el.querySelector("h4, h5, .product-name, [class*='name']")?.textContent || "").trim() || `NBN ${speedTier}`,
      speedTier,
      uploadSpeedMbps: extractUploadSpeed16(el, speedTier),
      dataAllowance: extractDataAllowance16(el),
      contractMonths: extractContractMonths16(el),
      modemIncluded: extractModemIncluded16(el),
      introPriceCents: introOffer.introPriceCents ?? parsePriceToCents(priceText),
      introDurationDays: introOffer.introDurationDays,
      ongoingPriceCents: introOffer.thenPriceCents ?? parsePriceToCents(priceText),
      minTermDays: null,
      setupFeeCents: extractSetupFee16(el),
      modemCostCents: null,
      conditionsText: (el.textContent || "").trim() || null,
      promoDescription: introOffer.promoDescription,
      typicalEveningSpeedMbps: null,
...
var init_iinet = __esm({
  "../../packages/shared/dist/parsers/providers/iinet.js"() {
    "use strict";
    init_validators();
    init_dom_utils();
    __name(extractUploadSpeed16, "extractUploadSpeed");
    __name(extractDataAllowance16, "extractDataAllowance");
    __name(extractContractMonths16, "extractContractMonths");
    __name(extractModemIncluded16, "extractModemIncluded");
    __name(extractSetupFee16, "extractSetupFee");
    __name(extractIntroOffer11, "extractIntroOffer");
    __name(canHandle16, "canHandle");
    __name(parse16, "parse");
  }
});

// ../../packages/shared/dist/parsers/providers/internode.js
var internode_exports = {};
__export(internode_exports, {
  canHandle: () => canHandle17,
  parse: () => parse17
});
function extractUploadSpeed17(planElement, downloadSpeed) {
  if (!downloadSpeed)
    return null;
  const planText = planElement.textContent || "";
  const uploadMatch = planText.match(/(\d+)\s*Mbps\s*(up|upload|↑)/i);
...
function extractDataAllowance17(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("unlimited"))
    return "Unlimited";
  const amountMatch = planText.match(/(\d+\.?\d*)\s*(tb|gb)/i);
...
function extractContractMonths17(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("no contract") || planText.includes("month-to-month"))
    return 0;
  if (planText.includes("24 month") || planText.includes("2 year"))
...
function extractModemIncluded17(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("modem included") || planText.includes("router included") || planText.includes("free modem"))
    return true;
  if (planText.includes("modem $") || planText.includes("bring your own") || planText.includes("your own modem"))
...
function extractSetupFee17(planElement) {
  const planText = planElement.textContent || "";
  if (planText.match(/free\s*(setup|connection|installation)/i))
    return 0;
  const feeMatch = planText.match(/(setup|connection|installation)[\s:]*\$(\d+(?:\.\d{2})?)/i);
...
function canHandle17(url) {
  return url.includes("internode.on.net") || url.includes("internode.com.au");
}
async function parse17(html, url) {
  const doc = parseHTML(html);
  const results = [];
  let rows = Array.from(doc.querySelectorAll(".plan, .broadband-plan, [class*='plan-'], .product"));
  if (rows.length === 0) {
    rows = Array.from(doc.querySelectorAll(".nbn, [class*='nbn'], [class*='broadband']")).filter((el) => /plan|speed|price/i.test(el.textContent || ""));
  }
  for (const el of rows) {
    const speedText = (el.textContent || "").match(/NBN\s*(\d{1,4})/i) || (el.textContent || "").match(/(\d{1,4})\s*Mbps/i);
    if (!speedText)
    ...
    if (!speedTier)
      continue;
    const priceText = el.querySelector(".price, [class*='price']")?.textContent || el.textContent || "";
    results.push({
      providerSlug: "internode",
      planName: (el.querySelector("h3, h4, .plan-name, [class*='name']")?.textContent || "").trim() || `NBN ${speedTier}`,
      speedTier,
      uploadSpeedMbps: extractUploadSpeed17(el, speedTier),
      dataAllowance: extractDataAllowance17(el),
      contractMonths: extractContractMonths17(el),
      modemIncluded: extractModemIncluded17(el),
      introPriceCents: parsePriceToCents(priceText),
      introDurationDays: null,
      ongoingPriceCents: parsePriceToCents(priceText),
      minTermDays: null,
      setupFeeCents: extractSetupFee17(el),
      modemCostCents: null,
      conditionsText: (el.textContent || "").trim() || null,
      typicalEveningSpeedMbps: null,
      sourceUrl: url
...
var init_internode = __esm({
  "../../packages/shared/dist/parsers/providers/internode.js"() {
    "use strict";
    init_validators();
    init_dom_utils();
    __name(extractUploadSpeed17, "extractUploadSpeed");
    __name(extractDataAllowance17, "extractDataAllowance");
    __name(extractContractMonths17, "extractContractMonths");
    __name(extractModemIncluded17, "extractModemIncluded");
    __name(extractSetupFee17, "extractSetupFee");
    __name(canHandle17, "canHandle");
    __name(parse17, "parse");
  }
});

// ../../packages/shared/dist/parsers/providers/westnet.js
var westnet_exports = {};
__export(westnet_exports, {
  canHandle: () => canHandle18,
  parse: () => parse18
});
function extractUploadSpeed18(planElement, downloadSpeed) {
  if (!downloadSpeed)
    return null;
  const planText = planElement.textContent || "";
  const uploadMatch = planText.match(/(\d+)\s*Mbps\s*(up|upload|↑)/i);
...
function extractDataAllowance18(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("unlimited"))
    return "Unlimited";
  const amountMatch = planText.match(/(\d+\.?\d*)\s*(tb|gb)/i);
...
function extractContractMonths18(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("no contract") || planText.includes("month-to-month"))
    return 0;
  if (planText.includes("24 month") || planText.includes("2 year"))
...
function extractModemIncluded18(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("modem included") || planText.includes("router included") || planText.includes("free modem"))
    return true;
  if (planText.includes("modem $") || planText.includes("bring your own") || planText.includes("your own modem"))
...
function extractSetupFee18(planElement) {
  const planText = planElement.textContent || "";
  if (planText.match(/free\s*(setup|connection|installation)/i))
    return 0;
  const feeMatch = planText.match(/(setup|connection|installation)[\s:]*\$(\d+(?:\.\d{2})?)/i);
...
function canHandle18(url) {
  return url.includes("westnet.com.au");
}
async function parse18(html, url) {
  const doc = parseHTML(html);
  const results = [];
  let rows = Array.from(doc.querySelectorAll(".plan, .plan-option, [class*='plan'], .nbn-plan"));
  if (rows.length === 0) {
    rows = Array.from(doc.querySelectorAll(".pricing, .product, [class*='pricing']")).filter((el) => /NBN|speed|plan/i.test(el.textContent || ""));
  }
  for (const el of rows) {
    const speedText = (el.textContent || "").match(/NBN\s*(\d{1,4})/i) || (el.textContent || "").match(/(\d{1,4})\s*Mbps/i);
    if (!speedText)
    ...
    const priceText = el.querySelector(".price, [class*='price']")?.textContent || el.textContent || "";
    results.push({
      providerSlug: "westnet",
      planName: (el.querySelector("h4, h5, .plan-name, [class*='title']")?.textContent || "").trim() || `NBN ${speedTier}`,
      speedTier,
      uploadSpeedMbps: extractUploadSpeed18(el, speedTier),
      dataAllowance: extractDataAllowance18(el),
      contractMonths: extractContractMonths18(el),
      modemIncluded: extractModemIncluded18(el),
      introPriceCents: parsePriceToCents(priceText),
      introDurationDays: null,
      ongoingPriceCents: parsePriceToCents(priceText),
      minTermDays: null,
      setupFeeCents: extractSetupFee18(el),
      modemCostCents: null,
      conditionsText: (el.textContent || "").trim() || null,
      typicalEveningSpeedMbps: null,
      sourceUrl: url
...
var init_westnet = __esm({
  "../../packages/shared/dist/parsers/providers/westnet.js"() {
    "use strict";
    init_validators();
    init_dom_utils();
    __name(extractUploadSpeed18, "extractUploadSpeed");
    __name(extractDataAllowance18, "extractDataAllowance");
    __name(extractContractMonths18, "extractContractMonths");
    __name(extractModemIncluded18, "extractModemIncluded");
    __name(extractSetupFee18, "extractSetupFee");
    __name(canHandle18, "canHandle");
    __name(parse18, "parse");
  }
});

// ../../packages/shared/dist/parsers/providers/localbroadband.js
var localbroadband_exports = {};
__export(localbroadband_exports, {
  canHandle: () => canHandle19,
  parse: () => parse19
});
function extractUploadSpeed19(planElement, downloadSpeed) {
  if (!downloadSpeed)
    return null;
  const planText = planElement.textContent || "";
  const uploadMatch = planText.match(/(\d+)\s*Mbps\s*(up|upload|↑)/i);
...
function extractDataAllowance19(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("unlimited"))
    return "Unlimited";
  const amountMatch = planText.match(/(\d+\.?\d*)\s*(tb|gb)/i);
...
function extractContractMonths19(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("no contract") || planText.includes("month-to-month"))
    return 0;
  if (planText.includes("24 month") || planText.includes("2 year"))
...
function extractModemIncluded19(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("modem included") || planText.includes("router included") || planText.includes("free modem"))
    return true;
  if (planText.includes("modem $") || planText.includes("bring your own") || planText.includes("your own modem"))
...
function extractSetupFee19(planElement) {
  const planText = planElement.textContent || "";
  if (planText.match(/free\s*(setup|connection|installation)/i))
    return 0;
  const feeMatch = planText.match(/(setup|connection|installation)[\s:]*\$(\d+(?:\.\d{2})?)/i);
...
function canHandle19(url) {
  return url.includes("localbroadband.com.au");
}
async function parse19(html, url) {
  const doc = parseHTML(html);
  const results = [];
  let rows = Array.from(doc.querySelectorAll(".plan, .bundle, [class*='plan-'], [data-plan]"));
  if (rows.length === 0) {
    rows = Array.from(doc.querySelectorAll("article, .product-section, [class*='product']")).filter((el) => /NBN|plan|speed/i.test(el.textContent || ""));
  }
  for (const el of rows) {
    const speedText = (el.textContent || "").match(/NBN\s*(\d{1,4})/i) || (el.textContent || "").match(/(\d{1,4})\s*Mbps/i);
    if (!speedText)
    ...
    const priceText = el.querySelector(".price, [class*='price']")?.textContent || el.textContent || "";
    results.push({
      providerSlug: "localbroadband",
      planName: (el.querySelector("h4, h5, .product-title, [class*='name']")?.textContent || "").trim() || `NBN ${speedTier}`,
      speedTier,
      uploadSpeedMbps: extractUploadSpeed19(el, speedTier),
      dataAllowance: extractDataAllowance19(el),
      contractMonths: extractContractMonths19(el),
      modemIncluded: extractModemIncluded19(el),
      introPriceCents: parsePriceToCents(priceText),
      introDurationDays: null,
      ongoingPriceCents: parsePriceToCents(priceText),
      minTermDays: null,
      setupFeeCents: extractSetupFee19(el),
      modemCostCents: null,
      conditionsText: (el.textContent || "").trim() || null,
      typicalEveningSpeedMbps: null,
      sourceUrl: url
...
var init_localbroadband = __esm({
  "../../packages/shared/dist/parsers/providers/localbroadband.js"() {
    "use strict";
    init_validators();
    init_dom_utils();
    __name(extractUploadSpeed19, "extractUploadSpeed");
    __name(extractDataAllowance19, "extractDataAllowance");
    __name(extractContractMonths19, "extractContractMonths");
    __name(extractModemIncluded19, "extractModemIncluded");
    __name(extractSetupFee19, "extractSetupFee");
    __name(canHandle19, "canHandle");
    __name(parse19, "parse");
  }
});

// ../../packages/shared/dist/parsers/providers/netspace.js
var netspace_exports = {};
__export(netspace_exports, {
  canHandle: () => canHandle20,
  parse: () => parse20
});
function extractUploadSpeed20(planElement, downloadSpeed) {
  if (!downloadSpeed)
    return null;
  const planText = planElement.textContent || "";
  const uploadMatch = planText.match(/(\d+)\s*Mbps\s*(up|upload|↑)/i);
...
function extractDataAllowance20(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("unlimited"))
    return "Unlimited";
  const amountMatch = planText.match(/(\d+\.?\d*)\s*(tb|gb)/i);
...
function extractContractMonths20(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("no contract") || planText.includes("month-to-month"))
    return 0;
  if (planText.includes("24 month") || planText.includes("2 year"))
...
function extractModemIncluded20(planElement) {
  const planText = (planElement.textContent || "").toLowerCase();
  if (planText.includes("modem included") || planText.includes("router included") || planText.includes("free modem"))
    return true;
  if (planText.includes("modem $") || planText.includes("bring your own") || planText.includes("your own modem"))
...
function extractSetupFee20(planElement) {
  const planText = planElement.textContent || "";
  if (planText.match(/free\s*(setup|connection|installation)/i))
    return 0;
  const feeMatch = planText.match(/(setup|connection|installation)[\s:]*\$(\d+(?:\.\d{2})?)/i);
...
function canHandle20(url) {
  return url.includes("netspace.net.au");
}
async function parse20(html, url) {
  const doc = parseHTML(html);
  const results = [];
  let rows = Array.from(doc.querySelectorAll(".plan, .plan-tier, [class*='plan-'], .package"));
  if (rows.length === 0) {
    rows = Array.from(doc.querySelectorAll(".pricing, .packages, [class*='pricing']")).filter((el) => /NBN|speed|plan/i.test(el.textContent || ""));
  }
  for (const el of rows) {
    const speedText = (el.textContent || "").match(/NBN\s*(\d{1,4})/i) || (el.textContent || "").match(/(\d{1,4})\s*Mbps/i);
    if (!speedText)
    ...
    const priceText = el.querySelector(".price, [class*='price']")?.textContent || el.textContent || "";
    results.push({
      providerSlug: "netspace",
      planName: (el.querySelector("h4, h5, .package-name, [class*='title']")?.textContent || "").trim() || `NBN ${speedTier}`,
      speedTier,
      uploadSpeedMbps: extractUploadSpeed20(el, speedTier),
      dataAllowance: extractDataAllowance20(el),
      contractMonths: extractContractMonths20(el),
      modemIncluded: extractModemIncluded20(el),
      introPriceCents: parsePriceToCents(priceText),
      introDurationDays: null,
      ongoingPriceCents: parsePriceToCents(priceText),
      minTermDays: null,
      setupFeeCents: extractSetupFee20(el),
      modemCostCents: null,
      conditionsText: (el.textContent || "").trim() || null,
      typicalEveningSpeedMbps: null,
      sourceUrl: url
...
var init_netspace = __esm({
  "../../packages/shared/dist/parsers/providers/netspace.js"() {
    "use strict";
    init_validators();
    init_dom_utils();
    __name(extractUploadSpeed20, "extractUploadSpeed");
    __name(extractDataAllowance20, "extractDataAllowance");
    __name(extractContractMonths20, "extractContractMonths");
    __name(extractModemIncluded20, "extractModemIncluded");
    __name(extractSetupFee20, "extractSetupFee");
    __name(canHandle20, "canHandle");
    __name(parse20, "parse");
  }
});

// ../../packages/shared/dist/parsers/generic.js
var generic_exports = {};
__export(generic_exports, {
  canHandle: () => canHandle21,
  parse: () => parse21
});
function canHandle21(_url) {
  return true;
}
async function parse21(html, url) {
  ...
  for (const chunk of chunks) {
    ...
    const speedMatch = chunk.match(/NBN\s*(\d{1,4})/i) || chunk.match(/(\d{1,4})\s*Mbps/i) || chunk.match(/(\d)\s*Gbps/i) || chunk.match(/(\d)Gbps/i);
    if (priceMatches.length === 0 || !speedMatch)
      continue;
    let speedValue = parseInt(speedMatch[1]);
    ...
    const speedTier = (() => {
      const allowed = [12, 25, 50, 100, 200, 250, 400, 500, 1e3, 2e3];
      return allowed.includes(speedValue) ? speedValue : null;
    })();
    if (!speedTier)
      continue;
    if (/what is|about|want to|download a hefty|stream your|in a world of|contact us|call now|sign up today/i.test(chunk) && chunk.length > 150)
      continue;
    const nameMatch = chunk.match(/^([^$\n]{10,80}?)[\s]*\$/);
    const planName = (nameMatch ? nameMatch[1].trim() : chunk.slice(0, 80).trim()).replace(/<[^>]+>/g, "").trim();
    if (planName.length < 3)
      continue;
    const uploadMatch = chunk.match(/(\d{1,4})\s*\/\s*(\d{1,4})\s*Mbps/i) || chunk.match(/Upload[:\s]+(\d{1,4})\s*Mbps/i);
    ...
    if (introMatch) {
      ...
      if (introPriceCents < 2e3) {
        introPriceCents = null;
      } else {
        introDurationDays = parseInt(introMatch[2]) * 30;
      }
    }
    if (ongoingMatch) {
    ...
    if (!introPriceCents && priceMatches.length === 1) {
      ...
      if (priceCents >= 2e3 && priceCents !== introPriceCents) {
        ongoingPriceCents = priceCents;
      }
    }
...
var init_generic = __esm({
  "../../packages/shared/dist/parsers/generic.js"() {
    "use strict";
    init_validators();
    __name(canHandle21, "canHandle");
    __name(parse21, "parse");
    __name(urlToSlug, "urlToSlug");
  }
});

// ../../packages/shared/dist/parsers/index.js
function findParserForUrl(url) {
  for (const p3 of PROVIDER_PARSERS) {
    try {
      if (p3.canHandle && p3.canHandle(url))
        return p3;
    } catch {
    }
  }
...
var PROVIDER_PARSERS;
var init_parsers = __esm({
  "../../packages/shared/dist/parsers/index.js"() {
    "use strict";
    init_aussiebroadband();
    init_spintel();
    init_dodo();
    init_telstra();
    init_optus();
    ...
    init_netspace();
    init_generic();
    PROVIDER_PARSERS = [telstra_exports, optus_exports, tpg_exports, vodafone_exports, superloop_exports, kogan_exports, foxtel_exports, aussiebroadband_exports, spintel_exports, dodo_exports, exetel_exports, arctel_exports, launtel_exports, leaptel_exports, myrepublic_exports, iinet_exports, internode_exports, westnet_exports, localbroadband_exports, netspace_exports];
    __name(findParserForUrl, "findParserForUrl");
  }
});
...
var scraper_api_exports = {};
__export(scraper_api_exports, {
  fetchViaScraperAPI: () => fetchViaScraperAPI,
  fetchWithFallback: () => fetchWithFallback,
  getScraperAPIStatus: () => getScraperAPIStatus
});
async function fetchViaScraperAPI(url, _apiKey, options = {}) {
  return await fetchWithRotation(url, options);
}
function fetchWithRotation(url, options = {}) {
  const userAgent = USER_AGENTS[Math.floor(Math.random() * USER_AGENTS.length)];
  const delay = options.delay ? Math.random() * 2e3 + 1e3 : 0;
...
async function fetchWithFallback(url, scraperApiKey, retryCount = 0) {
  const maxRetries = 3;
  const baseDelay = 1e3;
  try {
...
async function getScraperAPIStatus(_apiKey) {
  return {
...
var init_scraper_api = __esm({
  "src/lib/scraper-api.ts"() {
    ...
    __name(fetchViaScraperAPI, "fetchViaScraperAPI");
    __name(fetchWithRotation, "fetchWithRotation");
    __name(fetchWithFallback, "fetchWithFallback");
    __name(getScraperAPIStatus, "getScraperAPIStatus");
  }
});

// src/handlers/data-population.ts
var data_population_exports = {};
__export(data_population_exports, {
  getDataPopulationStatus: () => getDataPopulationStatus,
  handleDataPopulation: () => handleDataPopulation
});
async function handleDataPopulation(_env) {
  const db = await getDb();
  const batchSize = 3;
  console.log("\u{1F680} Starting Batch Data Population...");
  ...
  try {
    ...
    const verifiedRes = await verifiedStmt.first();
    ...
    const providers = getResults(providersRes);
    result.batchNumber = Math.ceil((result.totalProviders - result.verifiedOverall) / batchSize);
    console.log(`\u{1F4CA} Processing batch: ${result.batchNumber}/${Math.ceil(result.totalProviders / batchSize)}`);
    ...
    for (const provider of thisBatch) {
      ...
      try {
        console.log(`
\u{1F504} ${provider.slug} (${provider.canonical_url})`);
        ...
        await db.prepare(`
          UPDATE providers SET
            ...
            last_fetch_at = ?
          WHERE id = ?
        `).bind(errorMsg, (/* @__PURE__ */ new Date()).toISOString(), provider.id).run();
      }
    ...
    console.log("\n" + "=".repeat(60));
    console.log("\u{1F4CB} Batch Summary:");
    console.log(`   \u2705 This Batch: ${result.processedThisBatch}/${batchSize} providers`);
    console.log(`   \u{1F4CA} Plans Updated: ${result.processedPlansThisBatch}`);
    console.log(`   \u23F1\uFE0F  Duration: ${(result.duration / 1e3).toFixed(2)}s`);
    ...
    console.error("\u{1F4A5} Data population batch failed:", err);
    return {
      ok: false,
      error: err instanceof Error ? err.message : String(err),
      result
...
async function upsertPlanEnhanced(db, providerId, ext) {
  const now = (/* @__PURE__ */ new Date()).toISOString();
  ...
  ).bind(providerId, ext.planName).first();
...
async function getDataPopulationStatus() {
  ...
  `);
  const plansStats = await plansStmt.first();
  return {
    providers: {
...
var init_data_population = __esm({
  "src/handlers/data-population.ts"() {
    "use strict";
    init_db();
    init_dist();
    init_scraper_api();
    __name(handleDataPopulation, "handleDataPopulation");
    __name(upsertPlanEnhanced, "upsertPlanEnhanced");
    __name(getDataPopulationStatus, "getDataPopulationStatus");
    __name(getResults, "getResults");
  }
});

// src/lib/browser-rendering.ts
async function fetchViaBrowser(url, browserBinding) {
  let browser = null;
  let page = null;
  try {
    ...
    if (page) {
      try {
        ...
        console.error("Failed to close page:", e2);
      }
    }
    if (browser) {
      try {
        ...
        console.error("Failed to close browser:", e2);
      }
    }
  }
}
async function fetchWithSmartFallback(url, browserBinding) {
  const { fetchWithFallback: fetchWithFallback2 } = await Promise.resolve().then(() => (init_scraper_api(), scraper_api_exports));
  try {
    console.log(`Attempting user-agent rotation for ${url}...`);
    return await fetchWithFallback2(url);
  } catch (error) {
...
var init_browser_rendering = __esm({
  "src/lib/browser-rendering.ts"() {
    "use strict";
    __name(fetchViaBrowser, "fetchViaBrowser");
    __name(fetchWithSmartFallback, "fetchWithSmartFallback");
  }
});

// src/handlers/providers-fetcher.ts
async function fetchProvidersToUpdate(env, maxProviders = 5) {
  const db = await getDb();
...
async function processProvider(db, prov, result, env) {
  result.checked++;
  ...
  try {
    ...
    if (!preferBrowser && hash === prov.last_hash) {
      await db.prepare("UPDATE providers SET last_fetch_at = ?, last_error = NULL, needs_review = 0 WHERE id = ?").bind((/* @__PURE__ */ new Date()).toISOString(), prov.id).run();
    ...
    try {
      ...
      await db.prepare("UPDATE providers SET last_error = ?, last_fetch_at = ? WHERE id = ?").bind(String(err), (/* @__PURE__ */ new Date()).toISOString(), prov.id).run();
      await db.prepare("UPDATE providers SET needs_review = 1 WHERE id = ?").bind(prov.id).run();
      result.errors++;
      return;
    }
    if (!usedBrowser && env?.BROWSER) {
      const missingPrices = extracts.filter((e2) => !e2.ongoingPriceCents && !e2.introPriceCents).length;
    ...
    for (const e2 of extracts) {
      try {
        const normalized = normalizeExtract(e2);
    ...
    await db.prepare("UPDATE providers SET last_hash = ?, last_fetch_at = ?, last_error = NULL, needs_review = ? WHERE id = ?").bind(hash, (/* @__PURE__ */ new Date()).toISOString(), providerNeedsReview ? 1 : 0, prov.id).run();
    result.changed++;
  } catch (err) {
    result.errors++;
    const errorMsg = err instanceof Error ? err.message : String(err);
    ...
    await db.prepare("UPDATE providers SET last_error = ?, last_fetch_at = ?, needs_review = ? WHERE id = ?").bind(errorMsg, (/* @__PURE__ */ new Date()).toISOString(), needsReview, prov.id).run();
...
async function upsertPlan(db, providerId, ext) {
  const existing = await db.prepare("SELECT id FROM plans WHERE provider_id = ? AND plan_name = ?").bind(providerId, ext.planName).first();
  const now = (/* @__PURE__ */ new Date()).toISOString();
  const technologyType = ext.technologyType || "standard";
  const planType = ext.planType || "residential";
  const serviceType = ext.serviceType ?? "nbn";
...
var JS_RENDER_PROVIDERS;
var init_providers_fetcher = __esm({
  "src/handlers/providers-fetcher.ts"() {
    "use strict";
    init_db();
    init_dist();
    init_browser_rendering();
    JS_RENDER_PROVIDERS = /* @__PURE__ */ new Set([
      "telstra",
      "optus",
      "vodafone",
      "tpg",
    ...
    __name(fetchProvidersToUpdate, "fetchProvidersToUpdate");
    __name(processProvider, "processProvider");
    __name(computeHash, "computeHash");
    __name(upsertPlan, "upsertPlan");
  }
});
...
var init_mustache = __esm({
  "../../node_modules/.pnpm/mustache@4.2.0/node_modules/mustache/mustache.mjs"() {
    ...
    Writer.prototype.parse = /* @__PURE__ */ __name(function parse22(template, tags) {
    ...
    mustache.parse = /* @__PURE__ */ __name(function parse23(template, tags) {
...
// ../../node_modules/.pnpm/@cloudflare+ai@1.2.2/node_modules/@cloudflare/ai/dist/index.js
function r2(e2) {
  return Array.isArray(e2) || e2 instanceof n;
}
function a(e2) {
  return e2 instanceof n ? e2.length : e2.flat(1 / 0).reduce(((e3, t2) => e3 + (t2 instanceof n ? t2.length : 1)), 0);
}
function o2(e2, t2) {
  if (!r2(t2)) {
    switch (e2) {
      ...
      case s.Int32:
      case s.Uint32:
        if (Number.isInteger(t2)) return;
        break;
      case s.Int64:
      case s.Uint64:
        if ("bigint" == typeof t2) return;
        break;
      case s.String:
        if ("string" == typeof t2) return;
    }
    throw new Error(`unexpected type "${e2}" with value "${t2}".`);
  }
  t2.forEach(((t3) => o2(e2, t3)));
}
function i(e2, t2) {
  if (r2(t2)) return [...t2].map(((t3) => i(e2, t3)));
  switch (e2) {
    case s.String:
    case s.Bool:
    case s.Float16:
...
function E(e2, t2) {
  if (r2(t2)) return t2.map(((t3) => E(e2, t3)));
  switch (e2) {
    case s.String:
    case s.Bool:
    case s.Float16:
...
var e, s, n, p2, A, R, c, m, u2, O, l, I, D, y, W;
var init_dist2 = __esm({
  "../../node_modules/.pnpm/@cloudflare+ai@1.2.2/node_modules/@cloudflare/ai/dist/index.js"() {
    e = __toESM(require_base64_js(), 1);
    init_mustache();
    !(function(e2) {
    ...
    n = Object.getPrototypeOf(Uint8Array);
    __name(r2, "r");
    __name(a, "a");
    __name(o2, "o");
    __name(i, "i");
    ...
    p2 = class _p {
      ...
      constructor(e2, t2, s2 = {}) {
        this.type = e2, this.value = t2, s2.validate && o2(e2, this.value), void 0 === s2.shape ? r2(this.value) ? this.shape = [a(t2)] : this.shape = [] : this.shape = s2.shape, s2.validate && (function(e3, t3) {
          if (0 === e3.length && !r2(t3)) return;
          const s3 = e3.reduce(((e4, t4) => {
            if (!Number.isInteger(t4)) throw new Error(`expected shape to be array-like of integers but found non-integer element "${t4}"`);
            return e4 * t4;
          }), 1);
          if (s3 != a(t3)) throw new Error(`invalid shape: expected ${s3} elements for shape ${e3} but value array has length ${t3.length}`);
        })(this.shape, this.value), this.name = s2.name || null;
      }
      static fromJSON(e2) {
        const { type: t2, shape: s2, value: n2, b64Value: r3, name: a2 } = e2, o3 = { shape: s2, name: a2 };
        if (void 0 !== r3) {
          const e3 = (function(e4, t3) {
            const s3 = atob(e4), n3 = new Uint8Array(s3.length);
            for (let e5 = 0; e5 < s3.length; e5++) n3[e5] = s3.charCodeAt(e5);
            const r4 = new DataView(n3.buffer).buffer;
            switch (t3) {
              case "float32":
          ...
          return new _p(t2, e3, o3);
        }
        return new _p(t2, E(t2, n2), o3);
      }
      toJSON() {
    ...
    R = "Write code to solve the following coding problem that obeys the constraints and passes the example test cases. Please wrap your code answer using   ```:";
    c = /* @__PURE__ */ __name((e2, t2) => [{ role: "system", content: e2 }, { role: "user", content: t2 }], "c");
    m = /* @__PURE__ */ __name((e2) => {
      const t2 = {};
      e2.temperature && (t2.temperature = e2.temperature), e2.max_tokens && (t2.max_tokens = e2.max_tokens);
      const n2 = [new p2(s.String, [e2.prompt], { shape: [1], name: "text_input" }), new p2(s.String, [JSON.stringify(t2)], { shape: [1], name: "sampling_parameters" })];
      return e2.stream && n2.push(new p2(s.Bool, true, { name: "stream" })), e2.image && (n2.push(new p2(s.Uint8, e2.image, { shape: [1, e2.image.length], name: "image" })), n2.push(new p2(s.Bool, true, { name: "exclude_input_in_output" }))), n2;
    }, "m");
    u2 = /* @__PURE__ */ __name((e2, t2) => {
      let s2 = e2.generated_text.value[0];
      if (t2) for (const e3 in t2) s2 = s2.replace(t2[e3], "");
      return s2;
    }, "u");
    O = /* @__PURE__ */ __name((e2) => (e2.inputsDefaultsStream = { max_tokens: 1800, ...e2.inputsDefaultsStream || {} }, e2.inputsDefaults = { max_tokens: 256, ...e2.inputsDefaults || {} }, e2.preProcessingArgs = { promptTemplate: "bare", defaultContext: A, defaultPromptMessages: c, ...e2.preProcessingArgs || {} }, e2 = { type: "triton", ...e2 }), "O");
    l = /* @__PURE__ */ __name((e2) => (e2.inputsDefaultsStream = { max_tokens: 512, ...e2.inputsDefaultsStream || {} }, e2.inputsDefaults = { max_tokens: 512, ...e2.inputsDefaults || {} }, e2.preProcessingArgs = { promptTemplate: "bare", defaultContext: A, defaultPromptMessages: c, ...e2.preProcessingArgs || {} }, e2 = { type: "vllm", generateTensorsFunc: /* @__PURE__ */ __name((e3) => m(e3), "generateTensorsFunc"), postProcessingFunc: /* @__PURE__ */ __name((e3, t2) => e3.name.value[0].slice(t2.prompt.length), "postProcessingFunc"), postProcessingFuncStream: /* @__PURE__ */ __name((e3, t2, s2) => e3.name.value[0], "postProcessingFuncStream"), ...e2 }), "l");
    I = /* @__PURE__ */ __name((e2, t2, s2) => ({ type: "tgi", inputsDefaultsStream: { max_tokens: 512 }, inputsDefaults: { max_tokens: 256 }, preProcessingArgs: { promptTemplate: e2, defaultContext: t2, defaultPromptMessages: c }, postProcessingFunc: /* @__PURE__ */ __name((e3, t3) => u2(e3, s2), "postProcessingFunc"), postProcessingFuncStream: /* @__PURE__ */ __name((e3, t3, n2) => u2(e3, s2), "postProcessingFuncStream") }), "I");
    D = mustache_default.parse;
    y = mustache_default.render;
    TransformStream;
    TransformStream;
    I("deepseek", R, ["<|EOT|>"]), I("bare", R), I("inst", A), I("openchat", A), I("chatml", A, ["<|im_end|>"]), I("orca-hashes", A), I("llama2", A), I("zephyr", A), I("mistral-instruct", A), I("mistral-instruct", A), I("gemma", A), I("hermes2-pro", A), I("starling", A), I("llama2", R), l({ preProcessingArgs: { promptTemplate: "phi-2", defaultPromptMessages: /* @__PURE__ */ __name((e2, t2) => [{ role: "question", content: t2 }], "defaultPromptMessages") } }), l({ preProcessingArgs: { promptTemplate: "sqlcoder" } }), l({ preProcessingArgs: { defaultContext: "" } }), l({ preProcessingArgs: { promptTemplate: "falcon" } }), l({ preProcessingArgs: { promptTemplate: "chatml" } }), l({ preProcessingArgs: { promptTemplate: "chatml" } }), l({ preProcessingArgs: { promptTemplate: "chatml" } }), l({ preProcessingArgs: { promptTemplate: "chatml" } }), l({ preProcessingArgs: { promptTemplate: "chatml" } }), l({ preProcessingArgs: { promptTemplate: "tinyllama" } }), l({ preProcessingArgs: { promptTemplate: "openchat-alt" } }), l({ preProcessingArgs: { promptTemplate: "gemma" } }), l({ preProcessingArgs: { promptTemplate: "gemma" } }), l({ preProcessingArgs: { promptTemplate: "mistral-instruct" } }), l({ experimental: true, preProcessingArgs: { promptTemplate: "mistral-instruct" } }), l({ preProcessingArgs: { promptTemplate: "llama2" } }), l({ experimental: true, inputsDefaultsStream: { max_tokens: 1800 }, inputsDefaults: { max_tokens: 256 }, preProcessingArgs: { promptTemplate: "mistral-instruct" } }), l({ preProcessingArgs: { promptTemplate: "llama3" } }), l({ experimental: true }), l({ experimental: true }), l({ preProcessingArgs: { promptTemplate: "chatml" } }), l({ experimental: true }), O({ inputsDefaultsStream: { max_tokens: 2500 }, preProcessingArgs: { promptTemplate: "llama2" } }), O({ preProcessingArgs: { promptTemplate: "llama2" } }), O({ preProcessingArgs: { promptTemplate: "mistral-instruct" } });
    W = class {
      static {
        __name(this, "W");
      }
      ...
      async run(e2, t2) {
        const s2 = await this.binding.run(e2, t2, this.options);
...
// src/handlers/ai-summaries.ts
var ai_summaries_exports = {};
__export(ai_summaries_exports, {
  getBestDealsSummary: () => getBestDealsSummary,
  getPlanSummary: () => getPlanSummary,
  getProviderSummary: () => getProviderSummary,
  refreshAiSummaries: () => refreshAiSummaries
});
...
async function generatePlanSummaries(db, ai) {
  ...
  const plans = getResults2(planRows);
  let updated = 0;
...
Promo: ${plan.promo_code ?? "None"} ${plan.promo_description ?? ""}`;
    ...
    if (!summary) continue;
    await db.prepare(
      "INSERT INTO plan_ai_summaries (plan_id, summary, updated_at) VALUES (?,?,?) ON CONFLICT(plan_id) DO UPDATE SET summary = excluded.summary, updated_at = excluded.updated_at"
    ).bind(plan.id, summary, (/* @__PURE__ */ new Date()).toISOString()).run();
    updated++;
  }
  await logAiRun(db, "plan_summaries", "ok");
  return { updated, processed: plans.length };
}
async function generateProviderSummaries(db, ai) {
  ...
  const providers = getResults2(providerRows);
  let updated = 0;
  for (const provider of providers) {
...
Routing: ${provider.routing_info ?? "N/A"}`;
    ...
    if (!summary) continue;
    await db.prepare(
      "INSERT INTO provider_ai_summaries (provider_id, summary, updated_at) VALUES (?,?,?) ON CONFLICT(provider_id) DO UPDATE SET summary = excluded.summary, updated_at = excluded.updated_at"
    ).bind(provider.id, summary, (/* @__PURE__ */ new Date()).toISOString()).run();
    updated++;
  }
  await logAiRun(db, "provider_summaries", "ok");
  return { updated, processed: providers.length };
}
async function generateBestDealsSummary(db, ai) {
  ...
  const deals = getResults2(dealsRows);
...
${lines}`;
  ...
  if (summary) {
    await db.prepare(
      "INSERT INTO best_deals_summary (id, summary, updated_at) VALUES (1, ?, ?) ON CONFLICT(id) DO UPDATE SET summary = excluded.summary, updated_at = excluded.updated_at"
    ).bind(summary, (/* @__PURE__ */ new Date()).toISOString()).run();
  }
  await logAiRun(db, "best_deals", "ok");
  return { updated: summary ? 1 : 0, processed: deals.length };
}
async function getPlanSummary(req) {
  const url = new URL(req.url);
  const planId = url.searchParams.get("planId");
  if (!planId) return jsonResponse({ ok: false, error: "Missing planId" }, 400);
  const db = await getDb();
  const row = await db.prepare("SELECT summary, updated_at FROM plan_ai_summaries WHERE plan_id = ?").bind(planId).first();
  return jsonResponse({ ok: true, summary: row?.summary ?? null, updated_at: row?.updated_at ?? null });
}
async function getProviderSummary(req) {
  const url = new URL(req.url);
  const providerId = url.searchParams.get("providerId");
  if (!providerId) return jsonResponse({ ok: false, error: "Missing providerId" }, 400);
  const db = await getDb();
  const row = await db.prepare("SELECT summary, updated_at FROM provider_ai_summaries WHERE provider_id = ?").bind(providerId).first();
  return jsonResponse({ ok: true, summary: row?.summary ?? null, updated_at: row?.updated_at ?? null });
}
async function getBestDealsSummary() {
  const db = await getDb();
  const row = await db.prepare("SELECT summary, updated_at FROM best_deals_summary WHERE id = 1").first();
  return jsonResponse({ ok: true, summary: row?.summary ?? null, updated_at: row?.updated_at ?? null });
}
function getResults2(rows) {
  if (Array.isArray(rows)) return rows;
  return Array.isArray(rows.results) ? rows.results : [];
}
var PLAN_SUMMARY_MODEL, PROVIDER_SUMMARY_MODEL, DEALS_SUMMARY_MODEL, SUMMARY_MAX_AGE_DAYS;
var init_ai_summaries = __esm({
  "src/handlers/ai-summaries.ts"() {
    "use strict";
    init_dist2();
    init_db();
    init_cors();
    PLAN_SUMMARY_MODEL = "@cf/meta/llama-3.1-8b-instruct";
    PROVIDER_SUMMARY_MODEL = "@cf/meta/llama-3.1-8b-instruct";
    DEALS_SUMMARY_MODEL = "@cf/meta/llama-3.1-8b-instruct";
    SUMMARY_MAX_AGE_DAYS = 7;
    __name(pickAiText, "pickAiText");
    __name(formatMoney, "formatMoney");
    __name(logAiRun, "logAiRun");
    __name(refreshAiSummaries, "refreshAiSummaries");
    __name(generatePlanSummaries, "generatePlanSummaries");
    __name(generateProviderSummaries, "generateProviderSummaries");
    __name(generateBestDealsSummary, "generateBestDealsSummary");
    __name(getPlanSummary, "getPlanSummary");
    __name(getProviderSummary, "getProviderSummary");
    __name(getBestDealsSummary, "getBestDealsSummary");
    __name(getResults2, "getResults");
  }
});
...
async function handleCron(env) {
  ...
  try {
    ...
    if (env?.AI) {
      try {
        const { refreshAiSummaries: refreshAiSummaries2 } = await Promise.resolve().then(() => (init_ai_summaries(), ai_summaries_exports));
        await refreshAiSummaries2(env);
      } catch (err) {
...
async function debugProvider(slug) {
  const db = await getDb();
...
var init_debug = __esm({
  "src/handlers/debug.ts"() {
    ...
    init_db();
    init_dist();
    __name(debugProvider, "debugProvider");
  }
});

// src/handlers/admin-audit.ts
var admin_audit_exports = {};
__export(admin_audit_exports, {
  getAudit: () => getAudit
});
...
async function getAudit(_req) {
  ...
  const missingSpeed = await db.prepare("SELECT COUNT(*) as count FROM plans WHERE is_active = 1 AND (speed_tier IS NULL OR speed_tier <= 0)").first();
  const missingSpeedSample = await db.prepare(
    "SELECT id, plan_name, provider_id, speed_tier FROM plans WHERE is_active = 1 AND (speed_tier IS NULL OR speed_tier <= 0) LIMIT 20"
  ).all();
  const providersMissingMetadata = await db.prepare(
  ...
  const providersMissingMetadataSample = await db.prepare(
...
var init_admin_audit = __esm({
  "src/handlers/admin-audit.ts"() {
    ...
    init_cors();
    __name(toInt, "toInt");
    __name(getAudit, "getAudit");
  }
});

// ../../node_modules/.pnpm/itty-router@5.0.22/node_modules/itty-router/index.mjs
var t = /* @__PURE__ */ __name(({ base: e2 = "", routes: t2 = [], ...r3 } = {}) => ({ __proto__: new Proxy({}, { get: /* @__PURE__ */ __name((r4, o3, a2, s2) => (r5, ...c2) => t2.push([o3.toUpperCase?.(), RegExp(`^${(s2 = (e2 + r5).replace(/\/+(\/|$)/g, "$1")).replace(/(\/?\.?):(\w+)\+/g, "($1(?<$2>*))").replace(/(\/?\.?):(\w+)/g, "($1(?<$2>[^$1/]+?))").replace(/\./g, "\\.").replace(/(\/?)\*/g, "($1.*)?")}/*$`), c2, s2]) && a2, "get") }), routes: t2, ...r3, async fetch(e3, ...o3) {
  let a2, s2, c2 = new URL(e3.url), n2 = e3.query = { __proto__: null };
  for (let [e4, t3] of c2.searchParams) n2[e4] = n2[e4] ? [].concat(n2[e4], t3) : t3;
  e: try {
    for (let t3 of r3.before || []) if (null != (a2 = await t3(e3.proxy ?? e3, ...o3))) break e;
    t: for (let [r4, n3, l2, i2] of t2) if ((r4 == e3.method || "ALL" == r4) && (s2 = c2.pathname.match(n3))) {
      e3.params = s2.groups || {}, e3.route = i2;
      for (let t3 of l2) if (null != (a2 = await t3(e3.proxy ?? e3, ...o3))) break t;
    }
  } catch (t3) {
    if (!r3.catch) throw t3;
    a2 = await r3.catch(t3, e3.proxy ?? e3, ...o3);
  }
  try {
    for (let t3 of r3.finally || []) a2 = await t3(a2, e3.proxy ?? e3, ...o3) ?? a2;
  } catch (t3) {
    if (!r3.catch) throw t3;
    a2 = await r3.catch(t3, e3.proxy ?? e3, ...o3);
  }
...
var r = /* @__PURE__ */ __name((e2 = "text/plain; charset=utf-8", t2) => (r3, o3 = {}) => {
  if (void 0 === r3 || r3 instanceof Response) return r3;
  const a2 = new Response(t2?.(r3) ?? r3, o3.url ? void 0 : o3);
  return a2.headers.set("content-type", e2), a2;
}, "r");
...
try {
  router = t();
  router.get("/api/providers", async (req) => {
    const { getProviders: getProviders2 } = await Promise.resolve().then(() => (init_providers(), providers_exports));
    return getProviders2(req);
  });
  router.get("/api/providers/:slug", async ({ params }) => {
    const { getProvider: getProvider2 } = await Promise.resolve().then(() => (init_providers(), providers_exports));
    return getProvider2(params.slug);
  });
  router.get("/api/plans", async (req) => {
    const { getPlans: getPlans2 } = await Promise.resolve().then(() => (init_plans(), plans_exports));
    return getPlans2(req);
  });
  router.get("/api/plans/:id/history", async ({ params }) => {
    const { getPriceHistory: getPriceHistory2 } = await Promise.resolve().then(() => (init_price_history(), price_history_exports));
    return getPriceHistory2(params.id);
  });
  router.get("/api/price-history/:id", async ({ params }) => {
    const { getPriceHistory: getPriceHistory2 } = await Promise.resolve().then(() => (init_price_history(), price_history_exports));
    return getPriceHistory2(params.id);
  });
  router.get("/api/address/search", async (req) => {
    const { searchAddress: searchAddress2 } = await Promise.resolve().then(() => (init_address(), address_exports));
    return searchAddress2(req);
  });
  router.get("/api/address/qualify", async (req) => {
    const { qualifyAddress: qualifyAddress2 } = await Promise.resolve().then(() => (init_address(), address_exports));
    return qualifyAddress2(req);
  });
  router.get("/api/status", async (req, env) => {
    const { getStatus: getStatus2 } = await Promise.resolve().then(() => (init_status(), status_exports));
    return getStatus2(req, { CACHE: env.CACHE });
  });
  router.get("/api/admin/issues", async (req) => {
    const { getIssues: getIssues2 } = await Promise.resolve().then(() => (init_admin_issues(), admin_issues_exports));
    return getIssues2(req);
  });
  router.get("/api/admin/provider-verification", async (req) => {
    const { getProviderVerification: getProviderVerification2 } = await Promise.resolve().then(() => (init_provider_verification(), provider_verification_exports));
    return getProviderVerification2(req);
  });
  router.post("/api/admin/review/approve", async (request, env) => {
    const { adminApprove: adminApprove2 } = await Promise.resolve().then(() => (init_admin(), admin_exports));
    return adminApprove2(request, { ADMIN_TOKEN: env.ADMIN_TOKEN });
  });
  router.get("/api/providers/comparison", async (req, env) => {
    const { getProviderComparison: getProviderComparison2 } = await Promise.resolve().then(() => (init_provider_comparison(), provider_comparison_exports));
    return getProviderComparison2(req, env);
  });
  router.post("/api/savings/calculate", async (request, env) => {
    const { calculateSavings: calculateSavings2 } = await Promise.resolve().then(() => (init_savings_calculator(), savings_calculator_exports));
    return calculateSavings2(request, env);
  });
  router.post("/api/feedback", async (request, env) => {
    const { handleFeedback: handleFeedback2 } = await Promise.resolve().then(() => (init_feedback(), feedback_exports));
    return handleFeedback2(request, env);
  });
  ...
  router.post("/internal/update-favicons", async () => {
    try {
      const { updateProviderFavicons: updateProviderFavicons2 } = await Promise.resolve().then(() => (init_favicon(), favicon_exports));
      const result = await updateProviderFavicons2();
  ...
  router.get("/internal/verify-data", async (req) => {
    const { verifyData: verifyData2 } = await Promise.resolve().then(() => (init_data_verification(), data_verification_exports));
    return verifyData2(req);
  });
  router.post("/internal/data-population/populate", async (req, env) => {
    try {
      const { handleDataPopulation: handleDataPopulation2 } = await Promise.resolve().then(() => (init_data_population(), data_population_exports));
      const result = await handleDataPopulation2(env);
  ...
  router.get("/internal/data-population/status", async () => {
    try {
      const { getDataPopulationStatus: getDataPopulationStatus2 } = await Promise.resolve().then(() => (init_data_population(), data_population_exports));
      const result = await getDataPopulationStatus2();
      return new Response(JSON.stringify({ ok: true, result }), { status: 200, headers: { "Content-Type": "application/json" } });
    } catch (err) {
...
router.get("/internal/cron/run", async (req, env) => {
  try {
    const { handleCron: handleCron2 } = await Promise.resolve().then(() => (init_cron(), cron_exports));
    const result = await handleCron2(env);
    return new Response(JSON.stringify({ ok: true, result }), { status: 200, headers: { "Content-Type": "application/json" } });
  } catch (err) {
...
router.post("/internal/ai/refresh", async (_req, env) => {
  try {
    const { refreshAiSummaries: refreshAiSummaries2 } = await Promise.resolve().then(() => (init_ai_summaries(), ai_summaries_exports));
    const result = await refreshAiSummaries2(env);
    return new Response(JSON.stringify({ ok: true, result }), { status: 200, headers: { "Content-Type": "application/json" } });
  } catch (err) {
...
router.get("/api/ai/plan-summary", async (req) => {
  const { getPlanSummary: getPlanSummary2 } = await Promise.resolve().then(() => (init_ai_summaries(), ai_summaries_exports));
  return getPlanSummary2(req);
});
router.get("/api/ai/provider-summary", async (req) => {
  const { getProviderSummary: getProviderSummary2 } = await Promise.resolve().then(() => (init_ai_summaries(), ai_summaries_exports));
  return getProviderSummary2(req);
});
router.get("/api/ai/best-deals", async () => {
  const { getBestDealsSummary: getBestDealsSummary2 } = await Promise.resolve().then(() => (init_ai_summaries(), ai_summaries_exports));
  return getBestDealsSummary2();
});
router.get("/internal/debug/:slug", async ({ params }) => {
  try {
    const { debugProvider: debugProvider2 } = await Promise.resolve().then(() => (init_debug(), debug_exports));
    const out = await debugProvider2(params.slug);
...
router.get("/internal/ping", async () => {
  try {
    ...
    const r3 = await db.prepare("SELECT 1 as ok").first();
    return new Response(JSON.stringify({ ok: true, db: !!r3 }), { status: 200, headers: { "Content-Type": "application/json" } });
  } catch (err) {
...
router.get("/internal/env", async () => {
  try {
    ...
    const keys = Object.keys(globals).filter((k) => k.length < 60).slice(0, 200);
...
function errorJson(err, env) {
  const message = err instanceof Error ? err.message : String(err);
  const debug = isDebugEnabled(env);
  return {
    ok: false,
    error: message,
    ...debug ? { stack: err instanceof Error ? err.stack ?? null : null } : {}
  };
}
...
async function fetch2(request, env, _ctx) {
  const globals = globalThis;
  globals.D1 = env.D1;
  ...
  if (initError) {
    ...
    try {
      await recordRunError(String(msg));
    } catch (dbErr) {
      console.error("Failed to write run error to DB", dbErr);
    }
  ...
  if (pathname === "/internal/env") {
    try {
      const hasD1 = typeof globalThis.D1 !== "undefined";
      const keys = Object.keys(globalThis).filter((k) => k.length < 60).slice(0, 200);
      return new Response(JSON.stringify({ ok: true, hasD1, keys }), { status: 200, headers: { "Content-Type": "application/json" } });
    } catch (err) {
  ...
  if (pathname.startsWith("/internal/debug/")) {
    try {
      ...
      const { debugProvider: debugProvider2 } = await Promise.resolve().then(() => (init_debug(), debug_exports));
      const out = await debugProvider2(slug || "");
  ...
  if (pathname === "/internal/cron/run") {
    try {
      ...
      const result = await Promise.race([
        handleCron2(env),
        new Promise((_res, rej) => setTimeout(() => rej(new Error("Scraping timeout - operation took longer than 2 minutes")), 12e4))
      ]);
  ...
  if (pathname === "/api/providers") {
    try {
      const { getProviders: getProviders2 } = await Promise.resolve().then(() => (init_providers(), providers_exports));
      return await getProviders2(request);
    } catch (err) {
  ...
  if (pathname === "/api/plans") {
    try {
      const { getPlans: getPlans2 } = await Promise.resolve().then(() => (init_plans(), plans_exports));
      return await getPlans2(request, env);
    } catch (err) {
  ...
  if (pathname === "/api/ai/plan-summary") {
    try {
      const { getPlanSummary: getPlanSummary2 } = await Promise.resolve().then(() => (init_ai_summaries(), ai_summaries_exports));
      return await getPlanSummary2(request);
    } catch (err) {
  ...
  if (pathname === "/api/ai/provider-summary") {
    try {
      const { getProviderSummary: getProviderSummary2 } = await Promise.resolve().then(() => (init_ai_summaries(), ai_summaries_exports));
      return await getProviderSummary2(request);
    } catch (err) {
  ...
  if (pathname === "/api/ai/best-deals") {
    try {
      const { getBestDealsSummary: getBestDealsSummary2 } = await Promise.resolve().then(() => (init_ai_summaries(), ai_summaries_exports));
      return await getBestDealsSummary2();
    } catch (err) {
  ...
  if (pathname === "/api/address/search") {
    try {
      const { searchAddress: searchAddress2 } = await Promise.resolve().then(() => (init_address(), address_exports));
      return await searchAddress2(request);
    } catch (err) {
  ...
  if (pathname === "/api/address/qualify") {
    try {
      const { qualifyAddress: qualifyAddress2 } = await Promise.resolve().then(() => (init_address(), address_exports));
      return await qualifyAddress2(request);
    } catch (err) {
  ...
  if (priceHistoryMatch) {
    try {
      const { getPriceHistory: getPriceHistory2 } = await Promise.resolve().then(() => (init_price_history(), price_history_exports));
      return await getPriceHistory2(priceHistoryMatch[1]);
    } catch (err) {
  ...
  if (pathname === "/api/admin/review/approve" && request.method === "POST") {
    try {
      const { adminApprove: adminApprove2 } = await Promise.resolve().then(() => (init_admin(), admin_exports));
      return await adminApprove2(request, env);
    } catch (err) {
  ...
  if (pathname === "/api/admin/audit" && request.method === "GET") {
    try {
      const { getAudit: getAudit2 } = await Promise.resolve().then(() => (init_admin_audit(), admin_audit_exports));
      return await getAudit2(request);
    } catch (err) {
  ...
  try {
    const res = await Promise.race([
      ...
      new Promise((_, reject) => setTimeout(() => reject(new Error("router.handle timeout")), 15e3))
    ]);
    return res;
  } catch (err) {
    const msg = err instanceof Error ? err.stack || err.message : String(err);
...
async function scheduled(event, env, _ctx) {
  globalThis.D1 = env.D1;
  globalThis.ADMIN_TOKEN = env.ADMIN_TOKEN;
  if (initError) {
    const msg = initError.stack || initError.message || String(initError);
  ...
  try {
    ...
    const { handleCron: handleCron2 } = await Promise.resolve().then(() => (init_cron(), cron_exports));
    event.waitUntil(handleCron2(env));
  } catch (err) {
    const msg = err instanceof Error ? err.stack || err.message : String(err);
...
var index_default = { fetch: fetch2, scheduled };
export {
