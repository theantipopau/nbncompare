...
export async function getStatus(_req: Request, env?: { CACHE?: any }) {
  try {
    ...
    // Data quality checks
    const plansWithoutSourceUrl = await db.prepare("SELECT COUNT(*) as count FROM plans WHERE is_active = 1 AND (source_url IS NULL OR source_url = '')").first() as any;
    const plansWithoutPrice = await db.prepare("SELECT COUNT(*) as count FROM plans WHERE is_active = 1 AND (ongoing_price_cents IS NULL OR ongoing_price_cents <= 0)").first() as any;
    const plansWithoutSpeed = await db.prepare("SELECT COUNT(*) as count FROM plans WHERE is_active = 1 AND (speed_tier IS NULL OR speed_tier <= 0)").first() as any;
