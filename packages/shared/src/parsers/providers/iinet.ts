...
export async function parse(html: string, url: string): Promise<PlanExtract[]> {
  ...
  if (rows.length === 0) {
    // Fallback: look for sections with speed info
    rows = Array.from(doc.querySelectorAll("section, .pricing-section, [class*='plan-section']")).filter(el =>
      /NBN|speed|plan/i.test(el.textContent || "")
    );
  }
  ...
  for (const el of rows) {
    ...
    const introOffer = extractIntroOffer(el.textContent || "");
