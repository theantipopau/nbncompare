...
export async function parse(html: string, url: string): Promise<PlanExtract[]> {
  ...
  if (rows.length === 0) {
    // Fallback: look for product sections
    rows = Array.from(doc.querySelectorAll("article, .product-section, [class*='product']")).filter(el =>
      /NBN|plan|speed/i.test(el.textContent || "")
    );
  }
